...
- Packet coverage now distinguishes ZoneEngine server implementation from AOtomation message models. `AttackInfo` and `HealthDamage` are no longer treated as unknown wire shapes, but they remain message-model-only coverage in the current exporter. `CorpseFullUpdate` now has both an AOtomation message model and a ZoneEngine packet builder; the remaining gap is a first-class AOtomation serializer/lifecycle model instead of raw packet assembly.
...
- Loot data is structurally modeled in DB tables. The checked-in SQL seed still has no broad mob `DropHashes` assignments, but DB-backed corpse loot is now wired behind the same corpse session path. Runtime keeps deterministic `CombatTestLootCatalog` entries first for controlled test mobs, then falls back to parsed `mobtemplate`/`mobdroptable` entries for configured real mobs such as the local `cellao_codex_clean` `A004` Beach Leet setup.
...
2. Keep the focused corpse lifecycle smoke coverage locked: death registers the corpse before `CorpseFullUpdate`, `GenericCmd Use` opens loot, `ClientMoveItemToInventory` removes selected loot, inventory writes before marking loot consumed, and empty corpses use the short capture-backed despawn.
3. Expand DB loot coverage deliberately from reviewed captures. Keep generated review SQL unapplied until Mike approves the exact low-level mob assignments for `cellao_codex_clean` or the checked-in seed.
