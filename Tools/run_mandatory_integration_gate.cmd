...
for %%F in (
    ...
    "tools\DaoArchitectureGuard\dao_architecture_guard.py"
    "tools\DaoArchitectureGuard\known-violations.json"
...
if errorlevel 1 goto :stage_fail
dotnet run --project SharedBuild\Tools\RuntimeSourceGuard\RuntimeSourceGuard.csproj --configuration Release -- --repository-root . --self-test
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 2/12 engine management safety contracts

set "CURRENT_STAGE=3/12 DAO architecture guard"
echo [AORebirth Gate] START %CURRENT_STAGE%
call tools\run_dao_architecture_guard.cmd
if errorlevel 1 goto :stage_fail
...
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 5/12 complete AOtomation suite

set "CURRENT_STAGE=6/12 Subway acceptance"
echo [AORebirth Gate] START %CURRENT_STAGE%
call tools\run_subway_acceptance_tests.cmd
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 6/12 Subway acceptance

set "CURRENT_STAGE=7/12 Temple acceptance"
echo [AORebirth Gate] START %CURRENT_STAGE%
...
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 11/12 offline PHP/WebCore compatibility
