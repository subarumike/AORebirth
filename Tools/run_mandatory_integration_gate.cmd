...
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 3/12 generated artifact reproducibility

set "CURRENT_STAGE=4/12 complete AOtomation suite"
echo [AORebirth Gate] START %CURRENT_STAGE%
call tools\run_aotomation_messaging_tests.cmd
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 4/12 complete AOtomation suite

set "CURRENT_STAGE=5/12 Arete acceptance 60/60"
echo [AORebirth Gate] START %CURRENT_STAGE%
call tools\run_aotomation_messaging_tests.cmd /TestCaseFilter:"FullyQualifiedName~AreteAcceptanceMatrixTests"
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 5/12 Arete acceptance 60/60

set "CURRENT_STAGE=6/12 Arete combat catalog active coverage and loot"
echo [AORebirth Gate] START %CURRENT_STAGE%
...
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 6/12 Arete combat catalog active coverage and loot

set "CURRENT_STAGE=7/12 Subway acceptance"
echo [AORebirth Gate] START %CURRENT_STAGE%
call tools\run_subway_acceptance_tests.cmd
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 7/12 Subway acceptance

set "CURRENT_STAGE=8/12 Temple acceptance"
echo [AORebirth Gate] START %CURRENT_STAGE%
call tools\run_temple_acceptance_tests.cmd
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 8/12 Temple acceptance

set "CURRENT_STAGE=9/12 mission graph and generated mission reproducibility"
echo [AORebirth Gate] START %CURRENT_STAGE%
...
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 9/12 mission graph and generated mission reproducibility

set "CURRENT_STAGE=10/12 Git LFS integrity"
echo [AORebirth Gate] START %CURRENT_STAGE%
git lfs fsck
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 10/12 Git LFS integrity

set "CURRENT_STAGE=11/12 debug server build"
echo [AORebirth Gate] START %CURRENT_STAGE%
call tools\build_aorebirth_debug.cmd
if errorlevel 1 goto :stage_fail
echo [AORebirth Gate] PASS 11/12 debug server build

set "CURRENT_STAGE=12/12 clean worktree"
echo [AORebirth Gate] START %CURRENT_STAGE%
set "DIRTY=0"
for /f "delims=" %%G in ('git status --porcelain --untracked-files=all') do set "DIRTY=1"
if not "%DIRTY%"=="0" (
    echo [AORebirth Gate] FAIL 12/12 clean worktree - tracked or untracked output remains.
    goto :fail
)
echo [AORebirth Gate] PASS 12/12 clean worktree

echo [AORebirth Gate] PASS - all 12 mandatory stages completed.
popd >nul
exit /b 0
