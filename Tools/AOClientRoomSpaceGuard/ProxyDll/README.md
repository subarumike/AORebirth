...
The guard also prevents the null rectangle read captured in `Utils.dll +0x82F1`
from `GUI!View::ConvertToScreen`. It byte-verifies the exact supported GUI and
`Utils::Rect::operator+` sequence, then patches the normal path at
`GUI.dll +0x14C4A1`. A null scaled-rectangle input is replaced with the
already-computed scaled rectangle local before `operator+` is called. This
avoids exception dispatch entirely; the previous exception-resume recovery was
removed because a contained first fault could be followed immediately by
control-flow corruption. All other `Utils.dll` failures retain the normal crash
path.
