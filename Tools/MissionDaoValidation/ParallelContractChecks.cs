namespace AORebirth.Tools.MissionDaoValidation
{
    ...
    internal static partial class Program
    {
        ...
        private static void ValidateReadCardinalityAndParameters(IMissionDao dao)
        {
            ...
            // Current cutover requires a durable owner even for a mission-only snapshot.
            // The empty fixture character is seeded; absence is tested separately and fails closed.
            Expect<InvalidOperationException>(() => dao.ReadCharacter(10002), "snapshot-missing-owner-rejected");
            Expect<InvalidOperationException>(() => dao.Execute(10002, tx => true), "transaction-missing-owner-rejected");
        ...
        private static void ValidateCommitFailures(IMissionDao dao, string connectionString)
        {
            foreach (bool afterCommit in new[] { false, true })
            {
                ...
                try
                {
                    target.Execute(101, tx =>
                    {
                        escaped = tx;
                        tx.SaveMission(Key(mission.QuestId), mission);
                        return true;
                    });
                    Require(false, connection.FailurePoint + "-must-report-uncertain-outcome");
                }
                catch (MissionCommitOutcomeUnknownException exception)
                {
                    Require(ReferenceEquals(exception.InnerException, connection.OperationFailure),
                        connection.FailurePoint + "-preserves-provider-failure");
                }
                Require(mission.Version == 1, connection.FailurePoint + "-does-not-assume-rollback");
