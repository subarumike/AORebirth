namespace AORebirth.Tools.MissionDaoValidation
{
    ...
    internal static partial class Program
    {
        private static void ValidateHardening(IMissionDao dao, string connectionString)
        {
            ...
            run("rollback-error", () => ValidateOriginalFailure(connectionString));
        ...
        private static void ValidateOriginalFailure(string connectionString)
        {
            MySqlConnection connection = null;
            var original = new ExpectedRollbackException();
            try
            {
                new MySqlMissionDao(() => connection = new MySqlConnection(connectionString)).Execute<int>(101, tx =>
                {
                    tx.SaveMission(Key("hardening.rollback-transport"), NewMission("hardening.rollback-transport"));
                    // Closing a real provider connection loses the transaction transport before rollback.
                    connection.Close();
                    throw original;
                });
            ...
            catch (ExpectedRollbackException actual)
            {
                ...
                Require(actual.Data["MissionDao.RollbackFailure"] is Exception,
                    "rollback-failure-remains-inspectable");
            }
            Require(connection.State == ConnectionState.Closed, "failed-rollback-closes-connection");
            Require(Scalar(connectionString, "SELECT COUNT(*) FROM missionstates WHERE QuestId='hardening.rollback-transport'") == 0,
                "transport-loss-rolls-back-pending-write");
        }
