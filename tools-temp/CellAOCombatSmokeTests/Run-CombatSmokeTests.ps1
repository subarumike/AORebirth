...
Assert-SourceMatch $playfieldSource 'DoCombatTick\s*\(ICharacter\s+attacker\).*?TryLeashNpcAttacker\s*\(\s*attacker\s*\).*?return\s*;.*?!this\.IsInMeleeCombatRange' 'NPC leash checks should run before range-based pursuit/damage.'
Assert-SourceMatch $playfieldSource 'DoCombatTick\s*\(ICharacter\s+attacker\).*?!this\.IsInMeleeCombatRange\s*\(\s*attacker,\s*target\s*\).*?TryMoveNpcIntoCombatRange\s*\(\s*attacker,\s*target\s*\).*?nextCombatTicks\s*\[\s*attacker\.Identity\.Instance\s*\]\s*=\s*DateTime\.UtcNow\s*\+\s*TimeSpan\.FromSeconds\s*\(\s*1\s*\).*?return\s*;.*?int\s+currentHealth' 'Combat ticks should not deal melee damage while attacker and target are out of range.'
