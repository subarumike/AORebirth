...
Assert-SourceMatch $tradeHandlerSource 'PlayerTradeCredits\s*\(Identity\s+offerOwner,\s*int\s+credits\).*?x\.Action\s*=\s*TradeAction\.UpdateCredits.*?x\.Param1\s*=\s*credits;.*?x\.Param2\s*=\s*0;.*?x\.Param3\s*=\s*0;.*?x\.Param4\s*=\s*0;' 'Player trade credit notification should write the amount as the raw Param1 value, not as Target.Instance.'
