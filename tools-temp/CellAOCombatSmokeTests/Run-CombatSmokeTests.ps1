...
Assert-SourceMatch $statHandlerSource 'statsToClient\.TryGetValue\s*\(\s*\(int\)StatIds\.cash,\s*out\s+clientCash\s*\).*?clientCash\s*=\s*CashStatRules\.Normalize\s*\(\s*character\s*\).*?statsToClient\s*\[\s*\(int\)StatIds\.cash\s*\]\s*=\s*clientCash.*?this\.Send\s*\(\s*character,\s*this\.FillerBulk\s*\(\s*character,\s*statsToClient\s*\)\s*\)' 'Bulk cash stat sends should normalize authoritative cash before serializing to the client.'
Assert-SourceMatch $statHandlerSource 'SendSingle\s*\(ICharacter\s+character,\s*int\s+statId,\s*uint\s+statValue\).*?if\s*\(\s*statId\s*==\s*\(int\)StatIds\.cash\s*\).*?statValue\s*=\s*CashStatRules\.Normalize\s*\(\s*character\s*\).*?this\.Send\s*\(\s*character,\s*this\.Filler\s*\(\s*character,\s*statId,\s*statValue\s*\)\s*\)' 'Single cash stat sends should normalize authoritative cash before serializing.'
...
Assert-SourceMatch $tradeHandlerSource 'int\s+currentCash\s*=\s*GetCash\s*\(\s*client\.Controller\.Character\s*\)\s*;.*?long\s+finalCashRaw\s*=\s*\(long\)currentCash\s*-\s*cash\s*;.*?if\s*\(\s*finalCashRaw\s*<\s*0\s*\|\|\s*finalCashRaw\s*>\s*CashStatRules\.ClientSafeMaxCash\s*\).*?SetCash\s*\(\s*client\.Controller\.Character,\s*CashStatRules\.Clamp\s*\(\s*finalCashRaw\s*\)\s*\)\s*;.*?client\.Controller\.SendChangedStats\s*\(\s*\)' 'NPC/shop trade cash mutation should read through GetCash, validate and clamp, then SetCash and send changed stats.'
...
Assert-SourceMatch $clientConnectedSource 'Debug-only combat test mob spawns are disabled on normal login' 'ClientConnected should keep login-time combat-test spawns disabled.'
Assert-SourceNoMatch $clientConnectedSource 'EnsureDebugEnemySpawn|SendDebugEnemyToClient|DefaultForPlayfield\s*\(' 'ClientConnected should not run login-time debug enemy spawning; use explicit spawn commands.'
...
Assert-SourceNoMatch $playfieldSource 'AwardCorpseCredits\s*\(ICharacter\s+looter,\s*CorpseState\s+corpse\)(?:(?!private\s+void\s+SendCorpseLootAccessAction).)*?(SendCorpseCreditFeedback|ChatTextMessageHandler\.Default\.Send|You received \{0\} \{1\} from the corpse|Corpse credit feedback sent)' 'Corpse credit award should not send manual ChatText feedback; the client derives corpse credit text from the corrected corpse cash/stat flow.'
...
Assert-SourceMatch $corpseFullUpdateSource 'CorpseCashValueOffset\s*=\s*207.*?WriteInt32\s*\(\s*buffer,\s*CorpseCashValueOffset,\s*Math\.Max\s*\(\s*0,\s*corpseCredits\s*\)\s*\)' 'CorpseFullUpdate should write the registered corpse credit amount at offset 207 into the corpse dynel Cash stat instead of using the raw template value.'
...
Assert-SourceMatch $spawnCommandSource 'SpawnClientHintedMobs\s*\(ICharacter\s+character\).*?CombatTestMobArchetype\.ForPlayfield\s*\(\s*character\.Playfield\.Identity\.Instance\s*\).*?SpawnPopulationMob\s*\(.*?ZonePopulationOffsets.*?Spawned\s*\{0\}\s*DB population mobs' 'Spawn zone should create a DB-backed population pack from client-hinted playfield mappings.'
