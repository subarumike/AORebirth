...
Assert-SourceMatch $playfieldSource 'SendDebugCorpseFullUpdate\s*\(ICharacter\s+target,\s*Identity\s+corpseIdentity\).*?CorpseFullUpdate\.Build\s*\(\s*target,\s*corpseIdentity,\s*character\.Identity,\s*this\.server\.Id,\s*corpseCatMesh,\s*corpseMonsterData\s*\)' 'Playfield corpse lifecycle should delegate raw CorpseFullUpdate construction to the packet builder.'
