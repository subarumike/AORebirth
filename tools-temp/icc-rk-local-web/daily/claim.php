...
// Fallback: last character that zoned in (Zone writes last-active-account.json).
if ($accountKey === '') {
    $active = daily_read_first('last-active-account.json');
    if (is_array($active) && !empty($active['accountKey'])) {
        $accountKey = daily_safe_key($active['accountKey']);
        if ($accountKey === 'unknown') {
            $accountKey = '';
        }
    }
