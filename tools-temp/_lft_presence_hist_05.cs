namespace ZoneEngine.Core
{
    ...
    /// <summary>
    /// LFT Invite XP warning is client-side: GUI looks up a LOCAL dynel by character id.
    /// Cross-PF / out-of-range → "NoName is too high". Seed the official visibility
    /// sequence (SCFU + weapons + CharInPlay) from the REAL remote character onto the
    /// searcher's client, with name + XP-safe level.
    /// </summary>
    public static class LftInviteClientPresence
    {
            ...
            foreach (int candidateInstance in candidateInstances)
            {
                ...
                if (remote == null)
                {
                    LogUtil.Debug(
                        ...
                        "LFT seed miss id=" + candidateInstance + " searcher=" + searcher.Identity.Instance);
                    continue;
                }
        ...
        public static void SeedForInviteLookup(ICharacter requester, ICharacter remote, string nameOverride)
        {
            ...
            // Same/compatible → Level = searcher (never false "too high").
            // Really above XP window → real level (warn OK).
            bool reallyHigh = TeamXpShareWindow.IsTooHighForXpShare(requesterLevel, remoteLevel);
            int infoLevel = reallyHigh ? remoteLevel : requesterLevel;

            try
            {
                // Drop any stale/partial dynel for this id, then re-create with the
                // same packet pair the playfield uses for real same-PF visibility.
                SendToRequester(requester, DespawnMessageHandler.Default.Create(remote.Identity));

                SendVisibilityPair(requester, remoteChar, displayName, infoLevel);
                SendVisibilityPair(requester, remoteChar, displayName, infoLevel);
                ...
                ChatTextMessageHandler.Default.Send(
                    requester,
                    "LFT: prepared " + displayName
                    + (reallyHigh ? " (high — XP warn OK)" : " (same/near level)"));
            }
            catch (Exception ex)
            {
                LogUtil.Debug(
                ...
                ChatTextMessageHandler.Default.Send(
                    requester,
                    "LFT: seed FAILED for " + displayName + " — " + ex.Message);
            }
        }

        private static void SendVisibilityPair(
            ...
            int infoLevel)
        {
            ...
            {
                Type = IdentityType.CanbeAffected,
                Instance = remoteChar.Identity.Instance
            };
            ...
            {
                X = requesterCoord.x + 1.5f,
                Y = requesterCoord.y,
                Z = requesterCoord.z + 1.5f
            };

            SendToRequester(requester, scfu);

            foreach (WeaponItemFullUpdateMessage weapon in WeaponItemFullUpdate.CreateWeaponDefinitionMessages(remoteChar))
            {
                if (weapon != null)
                {
                    SendToRequester(requester, weapon);
                }
            }
        ...
        private static void SendToRequester(ICharacter requester, MessageBody body)
        {
            ...
            {
                // Official playfield fanout path (same as same-PF visibility).
