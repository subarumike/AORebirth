namespace ZoneEngine.Core
{
    ...
    /// <summary>
    /// Cross-zone LFT Invite needs id→name on the inviter client (else "NoName").
    /// Level in InfoPacket/SCFU is always the inviter level so the client XP
    /// "too high" dialog never appears. TeamXpShareWindow stays for LFT/XP share later.
    /// Same-PF: never seed (real dynel already there). Remote: off-map SCFU, no Despawn.
    /// </summary>
    public static class LftInviteClientPresence
    {
        ...
        /// <summary>
        /// Remote only: name via off-map SCFU + InfoPacket. Level always = inviter
        /// so client never shows XP "too high". No Despawn. Never invites.
        /// </summary>
        public static void SeedNameAndLevelOnly(ICharacter requester, ICharacter remote, string nameOverride)
        {
            ...
            // Always inviter level — removes client XP warn. Ranges kept in TeamXpShareWindow.
            int infoLevel = requesterLevel;
