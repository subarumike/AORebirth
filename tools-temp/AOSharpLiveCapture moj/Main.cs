...
namespace AOSharpLiveCapture
{
    public class Main : AOPluginEntry
    {
        ...
        private EventInfo chatMessageSentEvent;
        private Delegate chatMessageSentHandler;
        ...
        private const string TypedChatLogFileName = "AOSharpLiveCapture.typed-chat.log";
        private const string ChatSocketLogFileName = "AOSharpLiveCapture.chat-socket.log";
        ...
        private string typedChatLogPath;
        private long typedChatLogOffset;
        private DateTime nextTypedChatPollUtc;
        private string chatSocketLogPath;
        private long chatSocketLogOffset;
        private DateTime nextChatSocketPollUtc;
        private CombatLootSmoke combatLootSmoke;
        private MissionFlowCapture missionFlowCapture;
        private MissionInstanceCapture missionInstanceCapture;
        ...
        private void Initialize(string pluginDir)
        {
            ...
            this.typedChatLogPath = Path.Combine(Path.GetTempPath(), TypedChatLogFileName);
            this.ResetTypedChatLogOffset();
            this.chatSocketLogPath = Path.Combine(Path.GetTempPath(), ChatSocketLogFileName);
            this.ResetChatSocketLogOffset();
            ...
            this.missionInstanceCapture = new MissionInstanceCapture(this.LogEvent);
            this.missionInstanceCapture.BindSession(this.sessionDirectory);
            ...
            this.TrySubscribeChatMessageSent();
            ...
            this.LogEvent("PLUGIN", "Typed chat sidecar: " + this.typedChatLogPath);
            this.LogEvent("PLUGIN", "Chat socket sidecar: " + this.chatSocketLogPath);
            this.LogEvent("PLUGIN", "Mission flow log: " + Path.Combine(this.sessionDirectory, "mission-flow.log"));
            this.LogEvent("PLUGIN", "Mission instance log: " + Path.Combine(this.sessionDirectory, "mission-instance.log"));
            this.LogEvent("PLUGIN", "Mission instance map/doors/containers/spawns CSVs in session dir");
        ...
        private void TeardownCore()
        {
            ...
            this.callbackBoundary.Dispatch(
                "Teardown.MissionInstanceCapture",
                () => this.missionInstanceCapture?.Teardown());
            this.callbackBoundary.Dispatch(
        ...
        private void DisableAfterInitializationFailureNoThrow()
        {
            ...
            this.callbackBoundary.Dispatch(
                ...
                () => this.missionFlowCapture?.Teardown());
            this.callbackBoundary.Dispatch(
                "Run.InitializationFailure.MissionInstanceCapture",
                () => this.missionInstanceCapture?.Teardown());
            this.callbackBoundary.Dispatch(
        ...
        private void UnsubscribeCallbacksNoThrow()
        {
            ...
            this.callbackBoundary.Dispatch(
                ...
                () => Network.ChatMessageReceived -= this.OnChatMessageReceivedBoundary);
            this.callbackBoundary.Dispatch(
                "Unsubscribe.Network.ChatMessageSent",
                this.TryUnsubscribeChatMessageSent);
            this.callbackBoundary.Dispatch(
        ...
        private void OnChatMessageSentBoundary(object sender, ChatMessageBody message)
        {
            this.DispatchCallback(
                "Network.ChatMessageSent",
                () => this.OnChatMessageSent(sender, message));
        }
        ...
        private void OnN3MessageReceived(object sender, N3Message message)
        {
            lock (this.syncRoot)
            {
                ...
                this.RunN3CaptureStage(
                    ...
                    message,
                    "mission-instance",
                    () => this.missionInstanceCapture?.OnN3MessageReceived(message));
                this.RunN3CaptureStage(
                    "IN-N3",
                    sequence,
                    message,
        ...
        private void OnN3MessageSent(object sender, N3Message message)
        {
            lock (this.syncRoot)
            {
                ...
                this.RunN3CaptureStage(
                    ...
                    message,
                    "mission-instance",
                    () => this.missionInstanceCapture?.OnN3MessageSent(message));
                this.RunN3CaptureStage(
                    "OUT-N3",
                    sequence,
                    message,
        ...
        private void OnChatMessageSent(object sender, ChatMessageBody message)
        {
            lock (this.syncRoot)
            {
                if (!this.enabled || message == null)
                ...
                this.LogEvent("CHAT-OUT", this.DescribeObject(message));
                this.LogChatDialogue(
                    "CHAT-OUT",
                    0,
                    message.PacketType.ToString(),
                    "chat-protocol",
        ...
        private void OnPlayfieldInit(object sender, uint playfieldId)
        {
            ...
            this.missionInstanceCapture?.OnPlayfieldInit(playfieldId);
        ...
        private void OnUpdate(object sender, float deltaTime)
        {
            ...
            this.PollTypedChatLog(now);
            this.PollChatSocketLog(now);
            if (!this.enabled)
            {
                return;
            }
        ...
        private void PollTypedChatLog(DateTime now)
        {
            if (now < this.nextTypedChatPollUtc)
            {
                return;
            }

            this.nextTypedChatPollUtc = now.AddMilliseconds(250);
            if (!this.enabled || string.IsNullOrEmpty(this.typedChatLogPath))
            {
                return;
            }
            ...
            try
            {
                if (!File.Exists(this.typedChatLogPath))
                {
                    return;
                }

                using (FileStream stream = new FileStream(
                    this.typedChatLogPath,
                    FileMode.Open,
                    FileAccess.Read,
                    FileShare.ReadWrite))
                {
                    if (stream.Length < this.typedChatLogOffset)
                    {
                        this.typedChatLogOffset = 0;
                    }
                    ...
                    stream.Seek(this.typedChatLogOffset, SeekOrigin.Begin);
                    using (StreamReader reader = new StreamReader(stream, Encoding.UTF8, true, 1024, true))
                    {
                        ...
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            this.LogEvent("CHAT-TYPED", text);
                            this.LogChatDialogue("CHAT-TYPED", 0, "TypedChatInput", text, line);
                        }

                        this.typedChatLogOffset = stream.Position;
                    }
                }
            }
            catch (Exception ex)
            {
                this.LogEvent("CHAT-TYPED", "poll failed: " + OneLine(ex.Message));
            }
        }

        private void ResetTypedChatLogOffset()
        {
            try
            {
                if (!string.IsNullOrEmpty(this.typedChatLogPath) && File.Exists(this.typedChatLogPath))
                {
                    this.typedChatLogOffset = new FileInfo(this.typedChatLogPath).Length;
                }
                else
                {
                    this.typedChatLogOffset = 0;
            ...
            this.nextTypedChatPollUtc = DateTime.UtcNow;
        }

        private void PollChatSocketLog(DateTime now)
        {
            if (now < this.nextChatSocketPollUtc)
            {
                return;
            }

            this.nextChatSocketPollUtc = now.AddMilliseconds(250);
            if (!this.enabled || string.IsNullOrEmpty(this.chatSocketLogPath))
            {
                return;
            }
            ...
            try
            {
                if (!File.Exists(this.chatSocketLogPath))
                {
                    return;
                }

                using (FileStream stream = new FileStream(
                    this.chatSocketLogPath,
                    FileMode.Open,
                    FileAccess.Read,
                    FileShare.ReadWrite))
                {
                    if (stream.Length < this.chatSocketLogOffset)
                    {
                        this.chatSocketLogOffset = 0;
                    }
                    ...
                    stream.Seek(this.chatSocketLogOffset, SeekOrigin.Begin);
                    using (StreamReader reader = new StreamReader(stream, Encoding.UTF8, true, 1024, true))
                    {
                        string line;
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            string direction = "CHAT-SOCKET";
                            string detail = line;
                            string[] parts = line.Split(new[] { '\t' }, 4);
                            if (parts.Length >= 4)
                            {
                                direction = "CHAT-SOCKET-" + parts[1];
                                detail = "len=" + parts[2] + " hex=" + parts[3];
                            }

                            this.LogEvent(direction, detail);
                            this.LogChatDialogue(direction, 0, "ChatSocket", detail, line);
                        }

                        this.chatSocketLogOffset = stream.Position;
                    }
                }
            }
            catch (Exception ex)
            {
                this.LogEvent("CHAT-SOCKET", "poll failed: " + OneLine(ex.Message));
            }
        }

        private void ResetChatSocketLogOffset()
        {
            try
            {
                if (!string.IsNullOrEmpty(this.chatSocketLogPath) && File.Exists(this.chatSocketLogPath))
                {
                    this.chatSocketLogOffset = new FileInfo(this.chatSocketLogPath).Length;
                }
                else
                {
                    this.chatSocketLogOffset = 0;
            ...
            this.nextChatSocketPollUtc = DateTime.UtcNow;
        }
            ...
            byte[] packet)
        {
            ...
            if (decoded)
            {
                ...
                // Count any non-local SCFU with NPCInfo or PlayerInfo for structured export
                // (side NPCs like Marcus often arrive as PlayerInfo, not NPCInfo).
                if ((message.Npc != null || message.Player != null)
                    && this.IsNonLocalRawSimpleCharIdentity(message.Identity))
                {
            ...
            // Project NPCInfo and PlayerInfo SCFUs into enemy sinks so side/quest NPCs
            // (ICC Peacekeeper, Marcus Stone, etc.) get textures, ActiveNanos, and state.
            if (decoded
                && (message.Npc != null || message.Player != null)
                && this.IsNonLocalRawSimpleCharIdentity(message.Identity))
            {
        ...
        {
            SimpleCharInfo characterInfo = null;
            if (raw.Npc != null)
            {
            ...
            else if (raw.Player != null)
            {
                characterInfo = new SimpleCharInfo.PlayerInfo
                {
                    CurrentNano = raw.Player.CurrentNano,
                    Team = raw.Player.Team,
                    Swim = raw.Player.Swim,
                    StrengthBase = raw.Player.StrengthBase,
                    AgilityBase = raw.Player.AgilityBase,
                    StaminaBase = raw.Player.StaminaBase,
                    IntelligenceBase = raw.Player.IntelligenceBase,
                    SenseBase = raw.Player.SenseBase,
                    PsychicBase = raw.Player.PsychicBase,
                    FirstName = raw.Player.FirstName,
                    LastName = raw.Player.LastName,
                    OrgName = raw.Player.OrgName,
                    OrgId = raw.Player.OrgMetadata.GetValueOrDefault()
                };
            }
        ...
        private bool IsNonLocalRawSimpleCharIdentity(RawScfuIdentity identity)
        {
            if (identity.Type != (int)IdentityType.SimpleChar)
            {
                return false;
            }

            return !this.IsLocalPlayerIdentity(ToAoIdentity(identity));
        }
        ...
        private void LogN3Message(string direction, int sequence, N3Message message)
        {
            ...
            if (direction == "OUT-N3" || this.interestingMessageNames.Contains(messageName))
            {
                this.RunN3CaptureStage(
                    direction,
                    sequence,
        ...
        private void TrySubscribeChatMessageSent()
        {
            try
            {
                Type networkType = typeof(Network);
                this.chatMessageSentEvent = networkType.GetEvent("ChatMessageSent", BindingFlags.Public | BindingFlags.Static);
                if (this.chatMessageSentEvent == null)
                {
                    this.LogEvent("PLUGIN", "Network.ChatMessageSent not available; outbound chat logging disabled.");
                    return;
                }

                this.chatMessageSentHandler = Delegate.CreateDelegate(
                    this.chatMessageSentEvent.EventHandlerType,
                    this,
                    nameof(this.OnChatMessageSentBoundary),
                    false);
                if (this.chatMessageSentHandler == null)
                {
                    this.LogEvent("PLUGIN", "Network.ChatMessageSent exists but handler binding failed.");
                    return;
                }

                this.chatMessageSentEvent.AddEventHandler(null, this.chatMessageSentHandler);
                this.LogEvent("PLUGIN", "Network.ChatMessageSent subscribed.");
            }
            catch (Exception ex)
            {
                this.LogEvent("PLUGIN", "Network.ChatMessageSent subscribe failed: " + ex.Message);
            }
        }

        private void TryUnsubscribeChatMessageSent()
        {
            try
            {
                if (this.chatMessageSentEvent != null && this.chatMessageSentHandler != null)
                {
                    this.chatMessageSentEvent.RemoveEventHandler(null, this.chatMessageSentHandler);
                }
            }
            catch
            {
            ...
            {
                this.chatMessageSentEvent = null;
                this.chatMessageSentHandler = null;
            }
        }
        ...
        private static bool IsEnemyCombatEvidenceMessage(N3Message message)
        {
            ...
            return string.Equals(messageName, "Attack", StringComparison.OrdinalIgnoreCase)
                   ...
                   || string.Equals(messageName, "FightModeUpdate", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private bool TryRegisterFocusedEnemyFromMessage(string direction, int sequence, N3Message message)
        {
            ...
            if (TryGetIdentity(targetObject, out target))
            {
                ...
                // NPC↔NPC / NPC↔mob fights (Peacekeeper, Marcus ambient) never involve the local
                // player — still focus both SimpleChar parties so attack/anim evidence annotates.
                if (this.IsSimpleNonLocalCharacterIdentity(source)
                    && this.IsSimpleNonLocalCharacterIdentity(target))
                {
                    registered |= this.RegisterFocusedEnemyIdentity(source, "npc-combat-source", direction, sequence);
                    registered |= this.RegisterFocusedEnemyIdentity(target, "npc-combat-target", direction, sequence);
                }
            }
            ...
            RawSimpleCharFullUpdate message)
        {
            ...
            string characterInfoType = message.Npc != null ? "NPCInfo" : "PlayerInfo";
            string family = message.Npc != null
                                ? message.Npc.Family.ToString(CultureInfo.InvariantCulture)
                                : string.Empty;
            string losHeight = message.Npc != null
                                   ? message.Npc.LosHeight.ToString(CultureInfo.InvariantCulture)
                                   : string.Empty;
            string unknownData = message.Npc != null
                                     ? message.Npc.UnknownData.ToString(CultureInfo.InvariantCulture)
                                     : string.Empty;
            string unknownData2 = message.Npc != null
                                      ? message.Npc.UnknownData2.ToString(CultureInfo.InvariantCulture)
                                      : string.Empty;
            string unknownData3 = message.Npc != null && message.Npc.UnknownData3.HasValue
                                      ? message.Npc.UnknownData3.Value.ToString(CultureInfo.InvariantCulture)
                                      : string.Empty;
            if (message.Player != null)
            {
                family = message.Player.CurrentNano.ToString(CultureInfo.InvariantCulture);
                losHeight = message.Player.Team.ToString(CultureInfo.InvariantCulture);
                unknownData = message.Player.FirstName ?? string.Empty;
                unknownData2 = message.Player.LastName ?? string.Empty;
                unknownData3 = message.Player.OrgName ?? string.Empty;
            }
            ...
            string evidenceNotes = string.Format(
                CultureInfo.InvariantCulture,
                "raw-scfu flags={0} flags2={1} side={2} specialAttacks={3}",
                message.FlagsText,
                message.Flags2Text,
                message.AppearanceSide,
                RawScfuFormatting.FormatSpecialAttacks(message.SpecialAttacks));
        ...
        private void TrackEnemyStateFromMessage(string direction, int sequence, N3Message message)
        {
            ...
            if (attackInfo != null)
            {
                ...
                this.TrackEnemyCombatTarget(direction, sequence, attackInfo.Identity, "attack", false);
                return;
            }
            ...
            if (specialAttackInfo != null)
            {
                ...
                this.TrackEnemyCombatTarget(direction, sequence, specialAttackInfo.Identity, "special-attack", false);
                return;
            }
            ...
            if (attack != null)
            {
                ...
                this.TrackEnemyCombatTarget(direction, sequence, attack.Identity, "attack", false);
                return;
            }
            ...
            if (missedAttackInfo != null)
            {
                ...
                this.TrackEnemyCombatTarget(direction, sequence, missedAttackInfo.Identity, "miss", false);
                return;
            }
        ...
        {
            string actionName = GetMemberString(message, "Action");
            if (string.IsNullOrEmpty(actionName))
            {
                return;
            }

            bool isDeath = string.Equals(actionName, "Death", StringComparison.OrdinalIgnoreCase)
                           || string.Equals(actionName, "Die", StringComparison.OrdinalIgnoreCase);
            bool isCombatAnim = IsCombatOrAnimCharacterAction(actionName);
            if (!isDeath && !isCombatAnim)
            {
                return;
            }
            ...
            {
                // Still allow tracking when the actor is any non-local SimpleChar so
                // PlayerInfo side NPCs get attack/anim rows before SCFU population.
        ...
        private static bool IsCombatOrAnimCharacterAction(string actionName)
        {
            return string.Equals(actionName, "ChangeAnimationAndStance", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "SpecialUsed", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "SpecialAvailable", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "SpecialUnavailable", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "ItemAnim", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "NpcSocialAnim", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "FinishNanoCasting", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "InterruptNanoCasting", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "CastNano", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "SetNanoDuration", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "UsePerk", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(actionName, "ChangeVisualFlag", StringComparison.OrdinalIgnoreCase);
        }
        ...
        {
            // Any non-local SimpleChar SCFU (NPCInfo or PlayerInfo). Humanoid side NPCs
            // such as Marcus Stone often arrive as PlayerInfo and were previously dropped.
            return this.IsSimpleNonLocalCharacterIdentity(message.Identity);
        }
        ...
        private void DescribeIdentityForEnemyOutput(object identityValue, out string role, out string safeIdentity)
        {
            ...
            // Label any non-local SimpleChar combatant as enemy so NPC attack rows are not
            // dropped by SourceRole filters (PlayerInfo side NPCs were role=SimpleChar before).
            Identity identity;
            if (this.enemyStates.ContainsKey(identityText)
                || this.IsNpcDynelIdentity(identityValue)
                || (TryGetIdentity(identityValue, out identity) && this.IsSimpleNonLocalCharacterIdentity(identity)))
            {
                role = "enemy";
            }
        }
        ...
        private bool IsNpcDynelIdentity(object identityValue)
        {
            try
            {
                ...
                if (this.IsLocalPlayerIdentity(identity))
                {
                    return false;
                }
                ...
                if (dynel == null)
                {
                    // Unknown dynel: still treat non-local SimpleChar identities as NPC combatants.
                    return true;
                }
        ...
        private bool IsEnemyCharacter(SimpleChar character)
        {
            ...
            // Include side/quest NPCs that AOSharp reports as IsPlayer (Marcus) as well as IsNpc.
            return character.Identity.Type == IdentityType.SimpleChar;
        }
        ...
        private bool IsBroadVisibleEnemyEvidence(SimpleChar character)
        {
            ...
            // Always survey NPC/pet dynels (ICC Peacekeeper, wildlife, etc.).
            if (SafeBool(() => character.IsNpc) || SafeBool(() => character.IsPet))
            {
                return true;
            }

            // Humanoid side NPCs often report IsPlayer (Marcus). Avoid surveying every real
            // player on live; SCFU PlayerInfo still seeds tracking, then dynel updates apply.
            return this.IsFocusedEnemyIdentity(character.Identity)
                || this.IsTrackedEnemyState(character.Identity.ToString());
        }
        ...
        private CaptureValidation ValidateCapture()
        {
            ...
            if (this.enemyFullUpdateRowCount != this.rawNpcSimpleCharFullUpdateCount)
            {
                issues.Add(
                    string.Format(
                        CultureInfo.InvariantCulture,
                        "SCFU projection mismatch: decoded non-local NPC/Player SCFUs={0}, enemy-full-updates.csv rows={1}.",
                        this.rawNpcSimpleCharFullUpdateCount,
                        this.enemyFullUpdateRowCount));
            }
