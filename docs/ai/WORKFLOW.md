...
Capture-safe injection retains the standard AOSharp engine and network hooks but replaces AOSharp's native 131-byte chat GUI rewrite and `GetCommand` hook with one isolated chat-input hook after acquiring the per-client duplicate-injection guard. It recognizes only `/aocap` and `/aosmoke` and passes every other command to the client unchanged. Capture-safety contract 6 rejects zero, failed, or null-buffer chat socket reads and contains chat framing/parser exceptions inside the native receive hook, preventing managed `EndOfStreamException` failures from escaping into `AnarchyOnline.exe`. The native `StdString` allocation is fixed at the required 24-byte layout and deterministically disposed after every typed line. AOSharpLiveCapture itself signals readiness only after initialization and both command registrations succeed; the injector fails on a bounded readiness timeout instead of reporting a half-loaded capture, and disconnect unloads an unready Bootstrap so a retry is not blocked. Comprehensive capture starts automatically, and Mike can control it directly in game with:
