...
- Thief outgoing damage remains suppressed by default for PF127 MonsterData `26092`. A default-off, one-hit-only diagnostic gate `AO_REBIRTH_ENABLE_SUBWAY_THIEF_DIAGNOSTIC_DAMAGE` exists only for later proof that the original client accepts the repaired weapon context; it must not be enabled during the first non-damaging live-validation pass.
