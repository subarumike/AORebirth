...
- Guard telemetry identified the Subway client crash path as the client's Vehicle/N3 room-space query handling, but the current AORebirth-side trigger is captured PF127 NPC movement traffic. After moving Mpmaster out of PF127 cleared login, zoning back into Subway crashed while Zone was streaming captured `NPCCHASE` patrol/replay movement. Captured Subway NPC movement is disabled server-side until replay can be made safe; NPC spawn/content remains enabled.
