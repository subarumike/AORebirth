...
- Mike's `2026-07-12` live check proved the Thief fixed damage is landing as `9`, but the client rendered incoming hits as `nanobots / unknown damage`. Captured-equipped attack contracts now apply the equipped captured weapon's available display stats (`defaultattacktype`, `damagetype`, `weapontype`) to the NPC actor after equipping the weapon, limited to contracts that explicitly have captured equipped `AttackInfo`. This is intended to make Thief incoming hits render as projectile weapon damage while preserving the fixed captured `9` damage. Mike still needs to live-confirm the client text.
