...
- The first controlled ordinary weapon-hit evidence campaign is prepared but operator-pending. `tools\weapon_damage_evidence.cmd` creates ignored `.local\weapon-damage-evidence\<SESSION_ID>\` sessions, generated engine commands, manifests, operator steps, JSONL import, validation, and report-only candidate analysis. Runtime JSONL logging is disabled by default and only activates through `AO_REBIRTH_WEAPON_DAMAGE_EVIDENCE_SESSION`. No observations are collected yet, no formula is activated, and production damage remains unchanged.
