...
- Completed Thief-only capture `20260710-205400` now supplies the existing PF127 Thief with its repeated eight-leg walk cycle, a capture-matched `60`-second respawn after the dead NPC identity despawns (about `70.1` seconds after the observed death), exact SCFU appearance value `1576`, and the one-of-one observed QL1 Stolen Handbag corpse drop (`297055/297055`) scoped to PF127. The capture does not contain mission/dialogue packets, so active-mission drop gating and the associated quest chain remain unresolved. Focused tests, the Debug build, and Chat/Login/Zone restart pass; private-server kill, loot, and respawn smoke remains required.
