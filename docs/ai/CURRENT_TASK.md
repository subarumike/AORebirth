...
- Mike live-validated the next hallway Filth Flea on `2026-07-12` as working as intended for the tested gameplay path. Filth Flea also has captured corpse loot evidence in completed Subway captures `20260709-210452` and `20260709-220439`; those rows are now represented in the supported Subway loot table. Keep it as a passed hallway smoke result, but do not promote Filth Flea into `AcceptedSubwayEnemyGateRequiresWholeEnemyCoverage` until respawn/no-respawn expectations are explicitly covered by the accepted-enemy gate.
