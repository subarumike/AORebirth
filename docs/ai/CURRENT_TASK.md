...
- The centralized damage-calculation system now has a side-effect-free core boundary in `ZoneEngine.Core.DamageCalculator`, with deterministic random input, request/result/trace models, evidence classifications, fixed captured damage representation, and legacy normal-hit preservation through `CombatDamageRules`. The normal weapon-hit formula audit selected Outcome B: architecture and evidence only. Repository fields expose weapon min/max, legacy damage bonus, some AR/critical/stat contracts, and damage-type AC/add-damage mappings, but local evidence still does not prove attack-skill weighting, AR scaling, post-1000 behavior, AC division/order, critical formula, or AO add-damage ordering. Production damage therefore remains legacy/fixed, and candidate formula requests trace as evidence-blocked instead of activating speculative math. Current runtime callers still own health mutation, death, corpses, packets, aggro, loot, XP, ammo, recharge, movement, and timing.
