...
- Mike's `2026-07-12` live checks proved the Thief fixed damage path was landing `9`, but the client still rendered incoming hits as `nanobots / unknown damage` and the Thief did not visibly attack. Thief outgoing damage is now suppressed before damage application and before `AttackInfo` emission until the visible attack packet path is repaired. Player damage against the Thief, Thief spawn/visual, patrol, corpse, handbag loot, and respawn remain in scope for live validation.
