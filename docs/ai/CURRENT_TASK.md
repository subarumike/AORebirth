...
- The current supported-type population baseline contains 124 capture-backed Filth Flea, Discarded Pet, Disobedient Bot, Mugger, Thief, and Violent Vagabond spawns. Capture `20260710-202132` restores 29 exact supported-family source identities through the existing shared-family provider. Cross-capture duplicates remain excluded, and the captured identities/positions remain provenance inputs while the established runtime allocators continue producing collision-safe server identities.
- Packet-backed Mugger and Violent Vagabond visuals are applied from the recovered SCFU texture/mesh profiles.
- The ordinary capture-backed framework now adds 135 deduplicated PF127 spawns. Capture `20260710-202132` regenerates two Looters, six Stim Fiends, and one Deranged Shopper through the existing template-free ordinary provider. Deranged Shopper preserves its exact captured appearance, waypoints, and observed 9-point AttackInfo; its loot remains empty because no corpse loot was captured. Named bosses, `Healer`, owned summons, unsupported families, and duplicates remain excluded.
...
- Mike reports `20260709-222339` completes the remaining live Subway traversal. Follow-up capture `20260710-202132` completed cleanly and supplies the restored 38-row population slice. Its 107 unique SCFU identities are fully classified in `docs/generated/subway_20260710_population_restore_manifest.csv`: 29 supported-family restores, 9 ordinary regenerations, 18 duplicates, 2 owned summons, 49 unsupported-family exclusions, and no malformed rows. Historical commit `c2ebdb07` was used only as comparison evidence; overbroad rollback `e9405ab8` was not reverted. Named/boss evidence remains outside this slice.
