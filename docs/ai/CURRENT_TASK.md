...
- `Tools/AOClientRoomSpaceGuard/ProxyDll` now provides a RoomSpace-only `version.dll` proxy distribution lane for normal AO shortcuts. It accepts only the exact approved old/new `N3.dll` hashes, forwards the Windows version APIs, and patches the same four proven callsites in memory while leaving the EXE, `N3.dll`, resources, and shortcuts unchanged. Install/uninstall tooling refuses an existing `version.dll` conflict and carries the AOReloaded MIT attribution; AOReloaded's LAA patch, settings changes, and unrelated mods are excluded. The external guarded launcher remains the diagnostic/telemetry lane. Exact isolated install/uninstall smoke tests pass for both approved clients, and the verified package is installed on the old live client at `D:\Funcom\Anarchy Online`; pre/post hashes prove its EXE and `N3.dll` are unchanged. AO was not launched, so the next action is a normal live shortcut smoke confirming runtime `PATCH PASS` and room stability.
