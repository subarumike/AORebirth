...
The final validation receipt is `DELMUS_NPC_VALIDATION_RECEIPT.json`. Focused NPC/stat/weapon/content tests and nano-service tests pass. The standard Legacy build and accepted binding export pass (322 Subway plus 167 Temple bindings). An initial full run exhausted disk during duplicate playfield copying; only identical task-owned copies were replaced with hard links to the verified package. No user capture or source data was deleted. A cancellation regression found by the morph suite was corrected by retaining silent disconnect/lifecycle cancellation.

Final exact-source Windows acceptance for `b766f62235cd1c6dc5e201484925be70a8eb4017` passes, including 578 NewEngine tests. Exact-source Linux publication and packaged offline startup also pass on that source. All 12 mandatory gates passed on `22207743ed89034f6bd2072f37e4f2e0232accf6`, including 1,129 AOtomation tests, DAO architecture, Subway, Temple and mission gates; runtime and test source is unchanged between those commits. The later commit corrects the connected harness and completes reconciliation evidence. Publication validation does not establish live Linux/client acceptance or deployment approval. The final receipt commit changes documentation only.
...
The original receipt retained the zone-round-trip failure: expected Y=0, candidate Y=-0.0618746, baseline Y=-0.0620676. At that checkpoint the cause was unresolved and connected acceptance was FAIL. On 2026-09-14 the focused position task proved the first delta occurs in the existing motor's post-arrival ground-stick step, before persistence. The database preserves the final snapshot float exactly. The fixture now checks that authoritative snapshot, DAO reload and reconnect instead of requiring the original requested Y. Corrected control and candidate connected lifecycles pass, with all other durable fields still compared strictly. The runtime movement code is unchanged. Final source/gate evidence is recorded in `NEWENGINE_CONNECTED_POSITION_PERSISTENCE_RECONCILIATION.md` and its receipt; the earlier failed receipt remains historical evidence. NPC/content completion remains PARTIAL and is not an operational acceptance prerequisite.
