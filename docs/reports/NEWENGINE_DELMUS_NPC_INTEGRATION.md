...
After that harness correction, both binaries reach the same strict zone-round-trip persistence failure: expected Y=0, candidate Y=-0.0618746, baseline Y=-0.0620676. This small position drift between teleport and logout is consistent with active world simulation; its exact physics cause has not been isolated. The strict persistence comparison and gameplay physics were not changed. Connected acceptance remains **FAIL (reproduced on baseline)**, so this milestone is PARTIAL and is not deployment-ready. Cleanup reports no disposable container/network residue. Authentication, world entry and earlier persistence steps passed before this final failure; they do not override it.
