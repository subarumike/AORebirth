...
- The next final-commit stress attempt reached `sequential-check-seed-777` but
  all three formula-round-3 attempts exited with Windows access violation
  `0xC0000005` while iteratively skipping ItemDb values. At that point round 3's
  active bytes were already identical to round 2's active state, so the prior
  formula bytes were already the deterministic formula projection of that state.
