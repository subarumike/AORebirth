...
- Subway PF127 now has a 124-spawn supported-family baseline plus a template-free ordinary framework containing 135 deduplicated spawns. Completed capture `20260710-202132` restores 38 missing anchors: 29 Discarded Pet, Disobedient Bot, Mugger, and Violent Vagabond shared-family rows plus two Looters, six Stim Fiends, and one Deranged Shopper regenerated through the ordinary provider. The capture manifest accounts for all 107 unique SCFU identities and excludes duplicates, owned summons, named/boss content, and unsupported families. Exact captured source identities, positions, stats, appearances, and supported movement/combat evidence are preserved; runtime identities continue using the established collision-safe allocation paths. `c2ebdb07` was comparison evidence only and `e9405ab8` was not reverted wholesale. The repaired client RoomSpace issue is unchanged.
