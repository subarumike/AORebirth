...
- Global ordinary-world population now adapts all 259 Subway profile-backed rows into normalized spawn/group/respawn definitions. `WorldPopulationController` owns the 221 active rows, lifecycle state, and explicit death/despawn/corpse notifications; the same 38 rows remain disabled and quarantined. `WorldRespawnScheduler` replaces the ordinary runtime's local respawn dictionary and preserves the proven delay start at final dead-NPC despawn. Combat, movement, visibility, loot, corpse packets, identities/positions, Thief behavior, and Filth Flea behavior are unchanged. DB mobs and captured Arete robots remain documented legacy owners pending parity migration; no dyna content is active.
...
- Subway PF127 retains a 124-row supported-family evidence set plus 135 deduplicated ordinary evidence rows. Enabling all 38 candidate rows from capture `20260710-202132` reproduced the PF127 login crash during existing-character visibility delivery, matching the failure that caused the historical rollback. Those 29 supported-family and 9 ordinary rows are now runtime-quarantined while remaining checked in with their manifest and provenance; the active runtime returns to the prior 95 supported-family and 126 ordinary rows. Reintroduce the quarantined population only in bounded slices with a successful login check after each slice. The repaired client RoomSpace issue is unchanged.
...
- The PF127 diagnostic manifest and selectors remain available for `NONE`, `ALL_38`, `SUPPORTED_29`, `ORDINARY_9`, ordinal, identity, and family slices, but they control spawn eligibility only and cannot bypass spatial selection. The safe 221-row production disposition is unchanged. The 38 quarantined rows require bounded live login/traversal and lifecycle validation before the full 259-row population can be activated.
- Subway content work is the primary development track.
- AO client Subway room-space crash mitigation is A/B validated for both supported clients: the guarded old client remained stable on live while the unguarded control crashed immediately after guarded call `N3+0x148B6`, and the guarded new client remained stable on AORebirth while the unguarded control crashed immediately after guarded call `N3+0x16144`. Fresh telemetry correlates repeated Workman Striker queries to valid captured X/Z with client-derived heights `0.75` and `1.50` units below the actor, and a separate Infector query to valid X/Z with client-derived `Y=0`; all return room-cell `-1`. This proves the main failure is caller-side room-query construction/handling across multiple rooms, not a door, one enemy type, or bad AORebirth spawn coordinates. Server content remains unchanged; the guarded client safely returns no room, and its launcher owns startup so duplicate guard attempts cannot silently launch an unprotected client.
- The RoomSpace-only normal-shortcut repair at `Tools/AOClientRoomSpaceGuard/ProxyDll` is complete for the two approved client builds. The verified package is installed in `C:\Funcom\Anarchy Online` for local testing and `D:\Funcom\Anarchy Online` for live testing; pre/post SHA-256 checks prove both client EXEs and `N3.dll` files are unchanged, and Mike confirmed the guarded clients no longer crash. The proxy remains fail-closed for unknown hashes and the external guard remains available for diagnostics. Client-crash repair work is closed unless a regression appears; active development has returned to capture-backed PF127 Subway population and named/boss/archetype implementation from completed capture `20260709-222339`.
