...
- Grid terminal and grid movement repair: outside grid terminal entry routes are capture/user-evidence backed for the tested terminals, named grid-side anchors are mapped for Tir, Omni Trade, Newland, Old Athen, and Unicorn Defence Hub, and PF `152` grid level pads resolve `LineTeleport` destination playfield `0` to the current playfield. A 2026-06-22 private-server crash investigation found PF152 floor-pad `Terminal:C0160098` was still being handled as a full PF152-to-PF152 zone transfer; the fix changes it to a local current-playfield teleport. Temp-output ZoneEngine build passed and Mike live-smoked the Grid floor pad with no crash.
