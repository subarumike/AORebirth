...
- The completed 294-folder location inventory identifies 37 Subway-only
  ...
  empty startup remnants with no usable location evidence. The capture-backed
  deep-population slice restores 61 recurring enemy rows across Empty Shell,
  Fragmented Soul, Incomplete Rebuild, Melded Patterns, Molested Molecules,
  Premature Pattern, Redundant Scan, and Uncontrollable Anger. PF127 now has 321
  catalog rows, 283 active rows, and the same 38 quarantined diagnostic rows.
  Exact normal-hit runtime ranges exclude critical and player-owned-pet hits,
  and strict empty corpse snapshots now contribute to loot denominators.
  Eumenides, six capture-exact merchants, and the Slum Runner corpse/credit
  sample are now implemented from already-held PF127 evidence. See
  `docs/ai/CURRENT_TASK.md` and
...
- Eumenides is a separate named PF127 encounter built from atomic capture `20260716-034559`: L20, 2792 HP, MonsterData `203726`, scale `130`, RunSpeed `76`, exact appearance/flags/heading, QL20 weapon `123267/123268`, captured special-attack context, capture-bounded `15.609` proactive acquisition, shared LOS/navigation, and a `100`-unit leash. Runtime damage and recharge remain owned by the equipped item rather than hard-coded from the two observed `39/45` hits. Exact capture `20260716-222007` supplies the 416-byte CATMesh `17905` corpse and `186` credits. Item loot and active-nano refresh behavior remain unresolved. Because exact lifecycle timing is absent, the private named-enemy policy is explicitly ten-minute respawn, 30-minute loot-bearing corpse, and three-second empty cleanup rather than an official-live timing claim.
...
- Capture `20260709-212115` now materializes six exact Subway merchant NPC owners. Tailor, Weaponsdealer, Armorer, Pharmacist, and Tools expose five owner-linked vending machines containing all `140` captured rows in exact slot order. Container Supplier retains its captured appearance but intentionally has no shop endpoint because no stock snapshot or complete dialogue was captured. Direct use is identity- and playfield-checked, vendor full updates retain the NPC owner identity, and vendor cleanup follows playfield teardown.
...
- Finalized Slum Runner capture `20260716-034656` contributes six identity-linked corpse records with exact CATMesh `31774` and credits `144/144/144/131/137/131`. The profile preserves all six atomic observations, and the loot layer uses an explicit observed-samples policy so repeated sample values are not confused with the global unique `ObservedSet` contract. This replays the captured sample only; it does not claim an official distribution. Item loot and credit-to-level correlation remain unresolved.
