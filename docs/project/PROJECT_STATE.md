...
- Mike's `2026-07-12` Thief playtests confirmed the fixed `9` damage path was active but still produced bad incoming client text (`nanobots / unknown damage`) while the Thief did not visibly attack. Thief outgoing damage is now suppressed before damage application and before `AttackInfo` emission until the visible attack packet path is repaired; player damage against the Thief is unchanged.
