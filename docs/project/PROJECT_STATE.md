...
- Ordinary Subway enemies are represented by validated type profiles plus exact spawn rows and are consumed by one `OrdinaryEnemyRuntimeService`. The former supported-family and generated-ordinary spawn orchestrators are retired. Thief and Filth Flea use the shared path; all 259 existing evidence rows are represented, while the same 38 PF127 diagnostic rows remain quarantined and global visibility behavior is unchanged. See `docs/project/ORDINARY_ENEMY_RUNTIME.md`.
...
- PF127 visibility isolation is now operator-ready without changing the safe default population. The committed 38-row diagnostic manifest assigns stable ordinals and the opt-in `tools\subway_visibility_diagnostic.cmd` workflow selects `NONE`, `ALL_38`, `SUPPORTED_29`, `ORDINARY_9`, first-N, ordinal ranges, identity lists, or families. Initial PF127 visibility diagnostics record exact serializer-buffer sizes and actual transport completion for SCFU, weapon definitions, and CharInPlay into ignored session artifacts, while the analyzer preserves the distinction between last-completed identity and proven causality. The first controlled manual sequence remains `NONE`, `SUPPORTED_29`, and `ORDINARY_9`; no production cutoff, batching, throttling, pagination, or packet-field change has been made.
