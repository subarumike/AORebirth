...
- First-room Subway Thief behavior has been reintroduced as a bounded slice: captured patrol replay from `20260710-205400`, QL1 Stolen Handbag corpse loot, and 60-second respawn. Raw packet `CorpseFullUpdate #1580` from that capture proves the exact 412-byte corpse shape, CATMesh `5907`, MonsterData `26092`, and material tail; the server now uses that exact template instead of the crashing MonsterData-as-CATMesh fallback. Mike live-validated the interim suppressed-visual build did not crash on kill; the restored exact corpse visual and handbag interaction remain to be live-validated. The deferred mission chain is unchanged.
- AOSharpLiveCapture now has generic one-pass NPC lifecycle coverage: raw packet preservation, visible-character state, full updates, movement, combat/death, exact corpse full updates, corpse inventory, loot movement, corpse presence/despawn, and capture-completeness validation. `decode_npc_lifecycle_capture.py` retro-decodes existing folders so already-captured raw evidence is reusable.
- Captured enemy combat uses a shared atomic contract and runtime registry across supported and generated ordinary Subway populations. Each spawn declares fixed captured `AttackInfo`, captured equipped weapon, specialized captured behavior, or unresolved evidence; captured weapons are equipped during spawn, retaliation and attack-source selection use the registry, contracts are removed on despawn, and incomplete contracts are logged/refused. For Thief, live capture `20260711-170337` proves QL1 Solar-Powered Pistol `121567`, attack-context header `Unknown=0`, `SpecialAttackWeapon` body `32/32/32/32/0`, attack start after `1.409765s`, Target -> `StopMovingCmd` -> `SetPos` -> `NpcPath` after another `0.219999s`, first landed hit `11.409643s` after Thief attack start, fixed `9`-point normal `AttackInfo`, approximately six-second repeats, and `StopFight` immediately before Death. Private capture `20260711-172309` proves the pre-repair mismatch. The repair is build- and test-confirmed but still needs a post-repair live capture; Disobedient Bot and Lost Thought remain unresolved where completed captures contain no landed source evidence.
- Mike's `2026-07-12` Thief playtests confirmed the fixed `9` damage path was active but still produced bad incoming client text (`nanobots / unknown damage`) while the Thief did not visibly attack. The missing captured piece was the observer-facing NPC weapon definition: official live sends `WeaponItemFullUpdate` for the Thief's QL1 Solar-Powered Pistol immediately after the Thief SCFU, while AORebirth equipped the weapon server-side but sent the definition through the NPC object instead of announcing it to observing clients. Captured equipped Subway NPC weapon definitions are now announced to the playfield after SCFU, and the temporary Thief outgoing-damage suppression has been removed.
