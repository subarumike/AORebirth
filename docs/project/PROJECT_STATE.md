...
- Mike live-validated the next hallway Filth Flea on `2026-07-12` as working as intended for the tested gameplay path. This is a passed hallway smoke result, not yet an accepted-enemy promotion; current checked-in data has no Filth Flea loot definition or explicit respawn rule, so those expectations must be represented in `AcceptedSubwayEnemyGateRequiresWholeEnemyCoverage` before Filth Flea is marked fully accepted.
