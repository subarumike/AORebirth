...
- Grid terminal and grid movement repair: outside grid terminal entry routes are capture/user-evidence backed for the tested terminals, named grid-side anchors are mapped for Tir, Omni Trade, Newland, Old Athen, and Unicorn Defence Hub, and PF `152` grid level pads now use same-playfield `LineTeleport` destination handling. Mike live-smoked the PF `152` grid level pads as working on 2026-06-22.
