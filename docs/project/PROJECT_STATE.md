...
- The completed 294-folder location inventory identifies 37 Subway-only
  ...
  tool errors. The content ledger contains 23,993 rows while keeping 55
...
- The PF127 diagnostic manifest and selectors remain available for `NONE`, `ALL_38`, `SUPPORTED_29`, `ORDINARY_9`, ordinal, identity, and family slices, but they control spawn eligibility only and cannot bypass spatial selection. The safe production disposition is now 283 active rows. The 38 quarantined rows require bounded live login/traversal and lifecycle validation before the full 321-row population can be activated. The corpus audit also establishes that Bitaxel is a player (`PlayerInfo`, lifecycle `player=True`) rather than a missing enemy. Strike Foreman remains the genuine uncovered named enemy: appearance, weapon, raw attack initiation/chase context, corpse visual, and credits are exact, while outgoing damage/cadence, item loot, respawn, leash, and exact acquisition range remain unresolved. Container Supplier remains appearance-only because no stock update or dialogue exists in the corpus.
