...
- AORebirth build validation should use `tools\build_aorebirth_debug.cmd`; it clears stale compiler/build processes, builds `AORebirth.Core` before `ZoneEngine`, and uses single-node MSBuild with node reuse disabled. PowerShell and `.ps1` wrappers are deprecated for Codex build, launch, validation, and live capture workflows.
