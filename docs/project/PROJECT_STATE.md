...
- A reusable reviewed legacy first-open validator now promotes strict item denominators for Shadow (15 complete opens: eight positive/seven empty), ordinary Infector (seven: three/four), Architect Striker (four: three/one), and Melded Patterns (four: three/one). Every sample is tied to an exact corpse-full-update/dead-NPC generation and first raw `InventoryUpdate` before identity reuse; overlapping `20260709-212115/212336` Shadow projections count once, while unopened and snapshot-only corpses are excluded. Runtime uses independent observed-sample entries with `ItemPoolComplete=false`; these are private existing-capture replay weights, not official probability claims.
- Shadow, ordinary Infector, Architect Striker, Melded Patterns, and Workman Striker join the whole-enemy accepted set, bringing it to ten. Their accepted coverage binds exact spawns, appearance, capture-backed normal combat, shared chase, strict incomplete-pool loot, corpse visuals/credits, the private 240-second ordinary respawn policy, and shared corpse lifetimes. Shadow's two, Infector's three, Architect Striker's one, and Workman Striker's six observed critical outcomes remain report-only. Ordinary Infector stays isolated from Abmouth-owned specialization; Architect Striker keeps fixed captured combat without an invented weapon; Melded Patterns and all 21 Workman sources keep item-owned normal damage/recharge without invented attack context.
