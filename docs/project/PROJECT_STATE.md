...
- A RoomSpace-only normal-shortcut distribution lane now exists at `Tools/AOClientRoomSpaceGuard/ProxyDll`. The x86 `version.dll` proxy forwards the Windows version APIs, accepts only the exact approved old/new `N3.dll` hashes, and applies the same four proven callsite repairs in process memory. It leaves `AnarchyOnline.exe`, `N3.dll`, client resources, and shortcuts unchanged; refuses installation over an existing `version.dll`; includes install/uninstall tooling and AOReloaded MIT attribution; and excludes AOReloaded's LAA executable patch, settings changes, and unrelated mods. The external guarded launcher remains the diagnostic/telemetry lane. Exact isolated install/uninstall smoke tests pass for both supported clients. The verified package is installed on the old live client at `D:\Funcom\Anarchy Online`; pre/post SHA-256 checks prove `AnarchyOnline.exe` and `N3.dll` are unchanged. AO was not launched, so normal-shortcut runtime `PATCH PASS` and room-stability smoke remain pending.
