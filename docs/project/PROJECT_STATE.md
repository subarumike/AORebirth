...
- The capture-complete whole-enemy accepted set remains 21 of 26 ordinary profiles. Infected Attendant, Lost Thought, Empty Shell, Premature Pattern, and Violent Vagabond remain combat/loot-incomplete, but all 43 of their rows are active and the catalog has no quarantine. Their lifecycle is complete through the shared PF127 `240`-second respawn and `30/120/30` regular-corpse policy. The reviewed corpus evidence is indexed without target-role leakage: Infected has one local `11` outcome and six reviewed non-local attack starts; Lost has 11 other-player hits at `15..20` with a `4.5320703`-second median; Empty has local `15`, two misses, other-player `19`, hostile nanos `26414/81998/82482`, exact local nano-duration targets, and six reviewed proactive local starts; Premature has local normal `22`, local critical `41`, other-player `16`, player-pet `38`, and two reviewed proactive local starts. Their one-hit or non-local samples remain report-only and cannot become fixed runtime damage. Vagabond has 40 distinct local misses after overlap deduplication, a `4.5802404`-second median attempt cadence, and two exact simultaneous other-player attack starts. Its landed damage cannot be sampled against the current test character, so the user-approved private policy applies the same-level Mugger `9..12` normal range while retaining captured Vagabond cadence and packet shape. Runtime stays retaliatory with shared chase and no automatic radius. The captured `449.759588` interval and `1.088` position delta remain evidence but do not override the shared respawn policy. Held template `130590` is Red Wine and is rejected as a combat weapon. Genuine gaps now remain numeric reset/leash boundaries for all five; usable owner-linked weapon evidence for Infected/Lost/Empty/Premature; repeated local cadence/range for Infected/Empty/Premature; local landed damage for Lost; exact landed Vagabond parity beyond the approved policy; and strict attributable Empty/Premature loot.
