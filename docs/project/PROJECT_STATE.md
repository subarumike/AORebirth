...
- Production runs movement reset source `701478901f564b31abb5bbe8a96c74ccb05d794e`, pushed on `codex/reset-transfer-movement-input` and included in local master; remote master was not moved. Exact Windows/Linux acceptance passes (551 NewEngine tests each). Both engines pass runtime provenance and stability with zero restarts; ChatEngine/account broker remain active. Mike authorized graceful maintenance shutdown with players online; all 280 inventory rows and 28 characters remain present, with no schema or operator row changes. Mike's Fair Trade held-key/release visual retry remains pending. Online characters do not block explicitly authorized maintenance; the deployment README records the stopped-pair procedure. See `docs/reports/NEWENGINE_TRANSFER_MOVEMENT_RESET.md`.
