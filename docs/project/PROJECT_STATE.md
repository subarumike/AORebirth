...
- Ordinary Subway enemies are represented by validated type profiles plus exact spawn rows and are consumed by one `OrdinaryEnemyRuntimeService`. The former supported-family and generated-ordinary spawn orchestrators are retired. Thief, Filth Flea, and Bloodcreeper use the shared path; all 260 runtime population rows are represented, while the same 38 PF127 diagnostic rows remain quarantined. See `docs/project/ORDINARY_ENEMY_RUNTIME.md`.
- Every ordinary spawn now has one generic level definition: 259 remain capture-fixed and Bloodcreeper alone uses the configured inclusive `L15..L25` policy range. An injected selector resolves once per population generation before stats and combat preparation; visibility, combat reset, corpse state, and navigation cannot reroll it, while a new Bloodcreeper respawn generation can. The selected variant and generation remain attached to the runtime definition.
...
- Subway PF127 retains a 124-row supported-family evidence set plus 136 ordinary runtime rows. Enabling all 38 candidate rows from capture `20260710-202132` reproduced the PF127 login crash during existing-character visibility delivery, matching the failure that caused the historical rollback. Those 29 supported-family and 9 ordinary rows are now runtime-quarantined while remaining checked in with their manifest and provenance; the active runtime contains 95 supported-family and 127 ordinary rows. Reintroduce the quarantined population only in bounded slices with a successful login check after each slice. This server-side PF127 disposition is unchanged by the separate official live-client regression described below.
...
- The PF127 diagnostic manifest and selectors remain available for `NONE`, `ALL_38`, `SUPPORTED_29`, `ORDINARY_9`, ordinal, identity, and family slices, but they control spawn eligibility only and cannot bypass spatial selection. The safe production disposition is now 222 active rows. The 38 quarantined rows require bounded live login/traversal and lifecycle validation before the full 260-row population can be activated.
