...
- Loot-bearing corpse close/reopen is implemented from the official-live same-corpse `F6C004` sequence: open sends `InventoryUpdate`, close sends `CharacterAction 110` and the normal Use acknowledgement without an inventory refresh, and reopen sends `InventoryUpdate` with a refreshed handle. The rejected `0x66` and refresh-on-every-use paths are removed. Manual client validation is pending; the five-minute regular-corpse lifetime is unchanged.
- Regular corpses with an unlooted item or unclaimed credits receive a five-minute initial lifetime. Truly empty and fully looted corpses use the 30-second cleanup window; this protects ordinary enemy loot, including the first-room Subway Thief's guaranteed QL1 Stolen Handbag and Filth Flea credits, without leaving unlooted corpses indefinitely.
