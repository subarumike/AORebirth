...
- PF127 login heartbeat regression observed at `2026-07-10 22:17:05` is repaired at the exact derived-stat boundary. Template-free captured NPCs can lack the profession/breed inputs used by `StatMaxNanoEnergy`; nano regeneration now skips only those actors instead of throwing continuously and breaking world entry. The Debug build, test-assembly build, and Chat/Login/Zone restart pass; Mike's fresh PF127 login remains the live confirmation boundary.
...
- Subway PF127 now has a 124-spawn supported baseline plus a template-free ordinary framework containing 135 deduplicated Shadow, Stim Fiend, Workman/Architect Striker, Infected Attendant, Slum Runner, Looter, Infector, Lost Thought, Neural Burnout, and Deranged Shopper spawns. Completed capture `20260710-202132` adds 38 missing anchors across the seven user-reported families: 29 supported-family anchors, two Looters, six Stim Fiends, and one fully decoded Deranged Shopper. The implementation uses first exact SCFU canonical positions, preserves simultaneously observed close identities, corrects Disobedient Bot live family/flags, and retains Deranged Shopper's raw-SCFU appearance, waypoints, and observed `9`-point combat while leaving uncaptured loot empty. Focused tests, Debug build, and engine restart pass; private live traversal remains pending.
- Completed Thief capture `20260710-205400` extends the existing single Thief without adding a duplicate: its stable SCFU appearance value is `1576`, and it respawns `60` seconds after dead-NPC despawn. The captured repeated eight-leg patrol is retained as evidence but is not emitted at runtime while PF127 captured NPC movement is disabled for client-crash safety.
- Completed handbag-mission capture `20260710-212455` implements Natalia Akcora in PF655 and the captured `Mission:554D28C7` chain: exact SCFU placement/appearance, offer and turn-in options, captured QuestFullUpdate field data, mission-active PF127 Stolen Handbag loot `297055/297055`, atomic inventory turn-in, Daily Mission XP Reward item action `285612`, and captured action-59/quest-delete completion data. Hotfix `2026-07-10`: handbag QuestFullUpdate/quest-delete/completion UI packet emission is disabled by `EmitCapturedQuestUiPackets=false` after private-server login crashed while the same client logged into live; dialogue, active-session loot gating, trade consumption, and reward action remain wired. The capture proves a helper can land the final Thief hit, so mission loot checks for an active mission holder in PF127 rather than only the final attacker. Mission state remains process-local and the capture does not prove the player's pre-reward XP/token values; private acceptance, helper-kill loot, zone round-trip, turn-in, reward, relog smoke, and safe QuestFullUpdate parity remain pending.
- Subway content work is the primary development track.
- AO client Subway room-space crash mitigation is A/B validated for both supported clients: the guarded old client remained stable on live while the unguarded control crashed immediately after guarded call `N3+0x148B6`, and the guarded new client remained stable on AORebirth while the unguarded control crashed immediately after guarded call `N3+0x16144`. Fresh telemetry correlates repeated Workman Striker queries to valid captured X/Z with client-derived heights `0.75` and `1.50` units below the actor, and a separate Infector query to valid X/Z with client-derived `Y=0`; all return room-cell `-1`. A later private-server test proved login succeeds after moving Mpmaster out of PF127 but zoning into PF127 crashes while Zone streams captured `NPCCHASE` patrol/replay movement. Current server mitigation: keep captured Subway NPC spawns/content enabled but disable captured PF127 NPC movement emission until movement replay can be made client-safe.
- The RoomSpace-only normal-shortcut repair at `Tools/AOClientRoomSpaceGuard/ProxyDll` regressed in the first four-callsite proxy build. The new C client crashed before world entry with WER `anarchyonline.exe` heap-corruption `0xc0000374` in `ntdll.dll`; the report loaded `C:\Funcom\Anarchy Online\VERSION.dll`, while AORebirth logs showed only Login connect/disconnect and no Zone login. The proxy package is now narrowed to the one observed crashing RoomSpace callsite per supported client (`new N3+0x16144`, `old N3+0x148B6`), preserving the proven guarded repair while avoiding three unobserved early collision callsites. Package build/self-tests pass and the narrowed proxy is installed in both the C and D client folders. The external guard remains the broader diagnostic lane.
