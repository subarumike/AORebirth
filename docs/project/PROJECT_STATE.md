...
- AOSharpLiveCapture now has generic one-pass NPC lifecycle coverage: raw packet preservation, visible-character state with packet/source provenance, full updates, movement, combat/death, exact corpse full updates, corpse inventory, loot movement, corpse presence/despawn, same-archetype same-position respawn timing in `enemy-respawns.csv`, and capture-completeness validation. Respawn-marked captures validate incomplete unless a respawn is correlated. `decode_npc_lifecycle_capture.py` retro-decodes existing folders so already-captured raw evidence is reusable.
...
- Mike live-validated the next hallway Filth Flea on `2026-07-12` as working as intended for the tested gameplay path. Filth Flea has captured corpse loot evidence in completed Subway captures `20260709-210452` and `20260709-220439`, correlated from `inventory-updates.csv`, `enemy-combat.csv`, `enemy-dossier.json`, and `CorpseFullUpdate` corpse cash values; item loot is represented in the supported Subway loot table and observed nonzero corpse credits `29..79` are represented in `CombatCorpseRules`. This is a passed hallway smoke result, not yet an accepted-enemy promotion; respawn/no-respawn expectations must still be represented in `AcceptedSubwayEnemyGateRequiresWholeEnemyCoverage` before Filth Flea is marked fully accepted.
