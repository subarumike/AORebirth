...
- The first ordinary-hit evidence campaign is prepared for operator action. The repo-owned wrapper `tools\weapon_damage_evidence.cmd` creates ignored `.local\weapon-damage-evidence\<SESSION_ID>\` sessions, generated diagnostic engine commands, a primary QL1 Solar-Powered Pistol `121567` matrix, optional Worn Oak Bo `121565` confirmation fixture, an isolated `Malfunctioning Cleaning Robot` target procedure, JSONL import, validation, candidate comparison, and parity reports. Runtime logging is disabled by default through environment gating, no live observations have been collected, and no formula is activated.
