...
- Completed Thief capture `20260710-205400` extends the existing single Thief without adding a duplicate: its repeated eight-leg patrol is replayed, its stable SCFU appearance value is `1576`, it respawns `60` seconds after dead-NPC despawn, and PF127 uses the one-of-one observed QL1 Stolen Handbag item `297055/297055` before fallback loot tables. This one-corpse sample does not prove active-mission gating or a general drop probability, and no quest-chain behavior is implemented without mission/dialogue evidence. Focused tests, Debug build, and Chat/Login/Zone restart pass; private kill/loot/respawn smoke remains pending.
