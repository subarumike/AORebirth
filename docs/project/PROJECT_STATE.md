...
- Rex Larsson has a first controlled live dialogue routing adapter behind disabled-by-default environment gate `AO_REBIRTH_ENABLE_ARETE_REX_DIALOGUE_ROUTING`. When enabled, only `SimpleChar:782DE568` in playfield `6553 Arete Landing` loads the captured Rex Arete manifest and uses the existing KnuBot open/append-text/answer-list/close packet path to show captured dialogue prompt text/options and advance an in-memory dialogue session. Mission actions, objective progress, quest packets, rewards, inventory, XP, credits, DB writes, character stat mutation, action `59`, and `Quest Delete` interpretation remain disabled. Focused ZoneEngine build, Arete harness, and Rex dry-run pass; manual smoke confirmed the gated route opens and answer clicks are handled, with prompt text now recovered from raw `packets.hex.log` for the next in-client retest.
