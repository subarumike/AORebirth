...
- PF127 now has a promoted server collision asset from geometry-only safe capture `20260714-185728`, with fail-closed loading and segment/triangle queries. Captured combat contracts can require damage line of sight; Vergil Aeneid is the first enabled profile. Mike live-validated that Vergil no longer damages the player through Subway walls and resumes attacking when line of sight is clear. Wall-aware chase/path selection is not yet implemented: Vergil currently remains behind a blocking wall instead of routing around it, so that movement slice remains active.
