...
The latest completed vendor import milestone promoted the Neutral Training Startup Equipment live capture import, reducing actionable uncovered statel vendors from `29` to `27` after the Parnassos exclusion reduced the active capture backlog from `89` to `29`. Static vendor coverage data remains the active campaign area, because broad shop stock coverage is still documented as incomplete while transaction paths are verified.
