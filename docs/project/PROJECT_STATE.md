...
- Mike's live test confirms world entry and zoning on `517f5794`; server logs show successful PF800/PF1186 transfers and reconnects. An intermittent held-key running animation at Fair Trade exposed retained movement input across loading. The transfer reset is repaired in source with a reproducing regression test; it is not deployed yet. See `docs/reports/NEWENGINE_TRANSFER_MOVEMENT_RESET.md`.
