...
- Mike's live test confirms world entry and zoning on `517f5794`; server logs show successful PF800/PF1186 transfers and reconnects. An intermittent held-key running animation at Fair Trade exposed retained movement input across loading. Transfer reset source `70147890` is pushed on `codex/reset-transfer-movement-input`; exact Windows acceptance passes with 551 NewEngine tests. Linux acceptance and deployment remain pending. See `docs/reports/NEWENGINE_TRANSFER_MOVEMENT_RESET.md`.

- Production runs `517f579400b1901db2b4968d5c5393376791803e`: the character-selection/world-entry regression is repaired by restoring Legacy sparse stat defaults, vital ability trickle and the existing GM health rule, with final effective bounds enforced before spawn publication. Exact Windows and Linux acceptance pass (549 NewEngine tests each; 64 Linux deployment cases). LoginEngine/NewEngine runtime hashes, readiness and stability pass with zero restarts; ChatEngine/account broker remain healthy. All 280 inventory rows and 28 characters remain present; this repair made no schema or operator row changes. Source is pushed on `codex/bb5dd916-production-cutover` and included in local master; remote master was not moved. Mike's official-client world-entry retry remains pending. Receipt: `docs/reports/NEWENGINE_LIVE_CHARACTER_LOGIN_REPAIR.md`.
