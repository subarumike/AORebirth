...
- The whole-enemy accepted set is 21 of 26 ordinary profiles. Infected Attendant, Lost Thought, Empty Shell, Premature Pattern, and Violent Vagabond remain incomplete; their current dispositions total 32 active rows plus the 11 quarantined Vagabonds. The full 72-session Subway corpus audit found additional implementable evidence for all five, including local/other-player/pet target-role splits, aggression observations, Vagabond miss cadence and `459.978`-second respawn, strict loot where attributable, and exact corpse/credits. Genuine remaining gaps are numeric reset/leash boundaries for all five; respawn cycles for Infected/Lost/Empty/Premature; usable owner-linked weapon evidence for those four; repeated local cadence for Infected/Empty/Premature; local landed damage for Lost; any landed Vagabond damage semantics; and strict attributable Empty/Premature loot. Vagabond item `130590` is Red Wine, not usable weapon evidence.
