...
- PF127 has a promoted server collision asset from geometry-only safe capture `20260714-185728`, with fail-closed loading and segment/triangle queries. Captured combat contracts can require damage line of sight; Mike live-validated that Vergil no longer damages the player through Subway walls and resumes attacking when line of sight is clear. A global `ZoneEngine.Core.Navigation` hostile-NPC chase capability now owns provider-gated direct-segment checks, deterministic bounded grid/A* planning, cached route following, stuck/deviation recovery, retry suppression, and lifecycle cleanup. PF127/resource `127` is the first and only enabled geometry provider; every hostile NPC on the shared PF127 combat path inherits it, with Vergil as the representative end-to-end test. Other playfields preserve legacy chase until authoritative navigation input exists. PF127 currently supports same-elevation routing only because the promoted geometry does not prove walkable-floor projection or cross-elevation connectivity. Focused source/geometry tests pass; private live route/chase validation is pending, and no official AO pathfinding parity is claimed. See `docs/project/NPC_CHASE_NAVIGATION.md`.
...
- Subway PF127 retains a 124-row supported-family evidence set plus 135 deduplicated ordinary evidence rows. Enabling all 38 candidate rows from capture `20260710-202132` reproduced the PF127 login crash during existing-character visibility delivery, matching the failure that caused the historical rollback. Those 29 supported-family and 9 ordinary rows are now runtime-quarantined while remaining checked in with their manifest and provenance; the active runtime returns to the prior 95 supported-family and 126 ordinary rows. Reintroduce the quarantined population only in bounded slices with a successful login check after each slice. This server-side PF127 disposition is unchanged by the separate official live-client regression described below.
