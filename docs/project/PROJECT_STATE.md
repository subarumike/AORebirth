...
- The current loot slice activated no population rows; PF127 remains 260 catalog
  rows, 222 active rows, and 38 quarantined rows. The Bloodcreeper audit contains
  four exact corpse generations and two complete item inventories, both empty.
  No Bloodcreeper item identity or transfer is proven, so its item pool remains
  unresolved and inactive while the existing corpse, 150-credit, level, combat,
  and 240-second respawn slice awaits the bounded private smoke.
