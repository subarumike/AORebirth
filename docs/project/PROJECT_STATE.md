...
- Mike live-validated the next hallway Filth Flea on `2026-07-12` as working as intended for the tested gameplay path. Filth Flea has captured corpse loot evidence in completed Subway captures `20260709-210452` and `20260709-220439`, correlated from `inventory-updates.csv`, `enemy-combat.csv`, and `enemy-dossier.json`; that loot is now represented in the supported Subway loot table. This is a passed hallway smoke result, not yet an accepted-enemy promotion; respawn/no-respawn expectations must still be represented in `AcceptedSubwayEnemyGateRequiresWholeEnemyCoverage` before Filth Flea is marked fully accepted.
