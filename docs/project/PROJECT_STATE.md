...
- The completed 307-folder location inventory identifies 41 Subway-only
  captures, 31 mixed Subway/outside captures, 231 elsewhere captures, and four
  empty startup remnants with no usable location evidence. Of the 72
  Subway-bearing sessions, 69 contain actual raw packet rows. The earlier 65-row
  lifecycle set reprocesses with zero offline repairs, recapture requirements,
  or tool errors; the four new Eumenides sessions are indexed separately.
...
- Ordinary Subway enemies are represented by validated type profiles plus exact spawn rows and are consumed by one `OrdinaryEnemyRuntimeService`. The former supported-family and generated-ordinary spawn orchestrators are retired. Thief, Filth Flea, Bloodcreeper, and the eight restored deep recurring families use the shared path; all 321 runtime population rows are represented, with 310 active and only 11 evidence-incomplete Violent Vagabond rows quarantined. See `docs/project/ORDINARY_ENEMY_RUNTIME.md`.
- Legacy combat overlap is now handled explicitly rather than counted as independent evidence. Only declared simultaneous pairs `20260709-212115 -> 212336/213711` deduplicate within the audited 20-millisecond logger-skew boundary, using source, target, semantic target role, message, damage, slot, unknown, hit type, and weapon-instance shape; matching is one-to-one per supporting capture and never collapses events within one capture or unrelated sessions. Workman Striker now has 47 distinct normal local-player hits at `14..23`, six criticals at `36..42`, `5.092328`-second median cadence, and two separately classified Killer-pet hits. Architect Striker has 15 normal `13..17` hits, one `38` critical, and `5.425420`-second cadence.
- Workman Striker's accepted whole-enemy profile proves 21 active exact spawns across levels 13/14/15/16/17/25, exact CATMesh/credits for every spawn level, and ten deduplicated complete first corpse opens: eight positive and two explicit empty. Ten item/QL entries replay only their observed `1/10` or `2/10` frequencies, while ten unopened corpse generations are excluded and wider pool completeness remains unresolved. Runtime resolves the exact owner-linked captured low/high/QL weapon tuple for every active source identity and fails closed for missing, conflicting, unknown, or aggregate source selection. Equipped items own normal damage and recharge without synthetic fixed damage or AttackInfo context. A shared weapon-critical formula remains unproven, so six observed critical outcomes stay report-only and no formula is invented; acceptance guards all source weapons/spawns, shared chase, strict incomplete-pool loot, private respawn, and shared corpse lifetimes together.
...
- A reusable reviewed first-open validator now promotes 18 strict item denominators. In addition to Shadow, ordinary Infector, Architect Striker, and Melded Patterns, the recovered set is Mugger `17/3 empty`, Discarded Pet `16/3`, Stim Fiend `13/0`, Looter `11/5`, Violent Vagabond `11/1`, Bloodcreeper `4/3`, Infected Attendant `4/1`, Fragmented Soul `4/0`, Deranged Shopper `2/0`, Incomplete Rebuild `2/0`, Redundant Scan `2/1`, Uncontrollable Anger `2/0`, Lost Thought `1/0`, and Neural Burnout `4/2`. Exact capture/allocation allowlists and complete raw-generation fingerprints fail closed; declared overlap projections count once, while unopened, snapshot-only, and the known false Stim Fiend attribution remain excluded. Generated strict summary metadata drives `IndependentEntries`, observed empty counts, and `ItemPoolComplete=false` without a catalog MonsterData hardcode list. Empty Shell and Premature Pattern remain tableless; replay weights are private existing-capture policy, not official probability claims.
...
- Uncontrollable Anger is the twenty-first accepted ordinary profile. Six exact active rows preserve captured levels `13,13,19,20,23,23`, two patrol anchors, four static anchors, retaliatory shared chase, local-player SIW1 damage `11..18`, strict `2/0 empty` loot, CATMesh `96177`, inherited private respawn, and standard `3/240/3` corpse rules. Four Killer-pet hits at `25..42` and one other-player hit at `19` remain separate from the two local-player outcomes. The reviewed `20260709-222339` Killer window retains the complete `5.1165513`, `5.1671525`, `10.1003489` CSV interval series and uses the six-decimal median `5.167153`; the doubled interval is evidence, not divided inference. Six exact positive-credit corpses cover L11/L12/L13/L20/L21, while active L19 and L23 remain credit-unresolved rather than receiving an invented formula.
- The whole-enemy accepted set remains 21 of 26 ordinary profiles. Infected Attendant, Lost Thought, Empty Shell, Premature Pattern, and Violent Vagabond remain incomplete; their current dispositions total 32 active rows plus the 11 quarantined Vagabonds. The full 72-session corpus evidence is now indexed without target-role leakage: Infected has one local `11` outcome and six reviewed non-local attack starts; Lost has 11 other-player hits at `15..20` with a `4.5320703`-second median; Empty has local `15`, two misses, other-player `19`, hostile nanos `26414/81998/82482`, exact local nano-duration targets, and six reviewed proactive local starts; Premature has local normal `22`, local critical `41`, other-player `16`, player-pet `38`, and two reviewed proactive local starts. Their one-hit or non-local samples remain report-only and cannot become fixed runtime damage. Vagabond has 26 distinct local misses after overlap deduplication, `4.0799494`-second median attempt cadence, and two exact simultaneous other-player attack starts. Its automatic acquisition/chase evidence has a guaranteed `16.606338` lower bound but remains report-only while landed damage is unresolved; runtime stays retaliatory with shared chase and no automatic radius. The explicit `450`-second post-NPC-despawn policy comes from the exact `449.759588` interval and `1.088` position delta. Held template `130590` is Red Wine and is rejected as a combat weapon. Genuine remaining gaps are numeric reset/leash boundaries for all five; respawn cycles for Infected/Lost/Empty/Premature; usable owner-linked weapon evidence for those four; repeated local cadence/range for Infected/Empty/Premature; local landed damage for Lost; any landed Vagabond damage semantics; and strict attributable Empty/Premature loot.
- Shadow, ordinary Infector, Architect Striker, Melded Patterns, and Workman Striker join the confirmed whole-enemy accepted set, bringing it to ten. Looter, Bloodcreeper, Stim Fiend, and Neural Burnout bring the set to fourteen, Mugger is fifteenth, Deranged Shopper is sixteenth, Incomplete Rebuild is seventeenth, Redundant Scan is eighteenth, Fragmented Soul is nineteenth, Discarded Pet is twentieth, and Uncontrollable Anger is twenty-first. Looter resolves all eight exact owner-linked `123038/123039` weapon tuples by source identity and QL. Mugger resolves all nine current sources to QL1 `121567/121567`, carries only its captured AttackInfo fields, and lets the item own damage, damage bonus, and recharge; aggregate, missing, conflicting, or unknown selection fails closed. Its 38 normal `9..12` hits remain separate from three report-only `21` criticals. Deranged Shopper resolves only source `0x79574527` to QL8 `125454/125455`; its eight normal `9..15` hits stay separate from one report-only `27` critical and two captured misses. All accepted Looter, Mugger, Deranged Shopper, Stim Fiend, and Disobedient Bot rows are now active for bounded private validation. The gate binds exact population disposition, combat, strict incomplete loot, corpse visuals/credits, private respawn, and shared corpse lifetimes. Ordinary generation, the expanded gate, WorldPopulation `36/36`, named encounter `26/26`, and Subway loot `22/22` pass. ZoneEngine compiles; the approved wrapper's final copy remains blocked while the private server is running.
- Capture `20260709-212115` materializes six exact Subway merchant NPC owners. Tailor, Weaponsdealer, Armorer, Pharmacist, Tools, and Container Supplier expose six owner-linked vending machines containing all `202` captured rows in exact slot order. Container Supplier uses the exact 62-row `Cont` inventory captured on vending-machine template `99634` in `20260613-221619`; no dialogue is synthesized. Direct use is identity- and playfield-checked, vendor full updates retain the NPC owner identity, and vendor cleanup follows playfield teardown.
- Every ordinary spawn now has one generic level definition: 295 remain capture-fixed, Bloodcreeper uses its configured inclusive `L15..L25` policy range, ten Incomplete Rebuild sources use explicit source-local observed-variant pools totaling `23` atomic generations, four Redundant Scan sources use ten atomic generations, ten Fragmented Soul sources use `19` atomic generations, and Premature Pattern source `79545356` uses two stat-only atomic generations at L17/L18. An injected selector resolves once per population generation before stats and combat preparation; visibility, combat reset, corpse state, and navigation cannot reroll it, while a new respawn generation can. The selected variant and generation remain attached to the runtime definition.
...
- Premature Pattern source `79545356` now preserves the two exact same-source generations joined from captures `20260709-225408` and `20260712-232848`: L17/368 HP/scale 98/RunSpeed 65 and L18/394 HP/scale 98/RunSpeed 68. It also follows the complete reviewed out-and-back patrol. The variants intentionally have no weapon because neither capture contains a complete weapon update; uniform selection is private policy, no extra population row was added, and PF127 remains 321 total/310 active/11 quarantined.
...
- The PF127 diagnostic manifest retains its historical selector names `NONE`, `ALL_38`, `SUPPORTED_29`, `ORDINARY_9`, ordinal, identity, and family slices, but selectors control spawn eligibility only and cannot bypass spatial selection. The configured production disposition is now 310 active rows. The 16 rows activated in this slice require bounded live login/traversal/lifecycle validation; the 11 Violent Vagabonds remain quarantined until their whole-enemy evidence gaps are closed. The corpus audit also establishes that Bitaxel is a player (`PlayerInfo`, lifecycle `player=True`) rather than a missing enemy. Capture-local SCFU owner identities now preclassify player-owned pets before weaker enemy-role projections. Strike Foreman remains the genuine uncovered named enemy: appearance, QL19 weapon, raw special/attack initiation, chase start, corpse visual, and credits are exact. Raw capture `20260709-222339` also proves two normal `18` hits and one `40` critical against the non-local player Wardog at approximately five-second intervals; Wardog's SCFUs are `PlayerInfo`, `player=True`, and MonsterData `0`, so those post-mitigation other-player outcomes are neither local-player nor pet evidence. Killed source `7954512E` is bound to exact QL19 WeaponInstance `25713A73` and corpse `00F6E017`, while captured positions prove a `20.250672`-unit proactive-acquisition lower bound. Item loot, respawn, leash/reset, and the exact acquisition threshold/upper bound remain unresolved, so the encounter stays inactive. Container Supplier stock is resolved by exact template-`99634` evidence; dialogue remains unresolved.
