...
- Mike's `2026-07-12` Thief playtest confirmed the fixed `9` damage path is active but exposed bad incoming client text (`nanobots / unknown damage`). Captured-equipped attack contracts now apply the captured weapon's available display stats to the NPC actor after equipping the weapon, currently affecting the Thief's captured QL1 Solar-Powered Pistol path and awaiting live text confirmation.
