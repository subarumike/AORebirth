...
- Selective Delmus NPC candidate on `codex/delmus-npc-content-001`: 32 Subway templates are integrated as explicitly blocked content; missing family, weapon and official-placement mappings remain visible in per-NPC reports. AAAA cannot spawn as a combat actor. Capture-proven client interruption requests and movement-start server cancellation are wired into the existing nano service; outgoing reason-code policy remains unproven. Windows/unit/mandatory/publication gates pass, but connected acceptance reproduces an existing Y-position drift fixture failure. Production is unchanged; this candidate is not deployment-ready. See `docs/reports/NEWENGINE_DELMUS_NPC_INTEGRATION.md`.

- Production runs arrival-heading source `cb12160c37507f8318b7e9e39f424f9bf13faa23`, pushed on `codex/reset-transfer-movement-input` and included in local master; remote master was not moved. Door and explicit LineTeleport arrivals now face along the destination landing clearance; ordinary wall-border crossings preserve heading. Exact Windows/Linux acceptance passes (555 tests each; 64 Linux deployment and 8 artifact cases). Both engines pass runtime provenance and stability with zero restarts; ChatEngine/account broker remain active. All 280 inventory rows and 28 characters remain present, with no schema or operator row changes. Mike confirmed the reported arrival-facing issue fixed in the official client. See `docs/reports/NEWENGINE_ZONE_ARRIVAL_HEADING.md`.

- Movement reset source `701478901f564b31abb5bbe8a96c74ccb05d794e` is included in the current release and retained as rollback. It clears stale motor inputs on zoning. Online characters do not block explicitly authorized maintenance; the deployment README records the graceful stopped-pair procedure. See `docs/reports/NEWENGINE_TRANSFER_MOVEMENT_RESET.md`.

- Prior production source `517f579400b1901db2b4968d5c5393376791803e`, retained as rollback and included in the current build, repaired character-selection/world-entry by restoring Legacy sparse stat defaults, vital ability trickle and the existing GM health rule, with final effective bounds enforced before spawn publication. Exact Windows and Linux acceptance passed (549 NewEngine tests each; 64 Linux deployment cases). Mike subsequently confirmed world entry and zoning, including PF800/PF1186 transfers. Receipt: `docs/reports/NEWENGINE_LIVE_CHARACTER_LOGIN_REPAIR.md`.

- Original `bb5dd916` production cutover: all four approved inventory/mission migrations passed, preserving 280 inventory rows and all 40 original tables. Verified backup restoration and database rollback were proven in isolation; no production restoration was needed. The retained prior NewEngine release and rollback evidence remain available. Original build acceptance: Windows PASS (544 NewEngine, 1,129 AOtomation, 12 mandatory gates); Linux PASS (544 NewEngine). Receipts: `docs/reports/BB5DD916_PRODUCTION_CUTOVER_RECEIPT.md` and `docs/reports/NEWENGINE_MASTER_LINUX_BUILD_RECEIPT.md`.
