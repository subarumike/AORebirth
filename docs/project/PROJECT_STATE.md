...
- First-room Subway Thief behavior has been reintroduced as a bounded slice: captured patrol replay from `20260710-205400`, logical corpse registration, QL1 Stolen Handbag corpse loot, and 60-second respawn. The unresolved corpse visual is suppressed because the capture proves inventory but contains no identity-linked `CorpseFullUpdate`; using MonsterData `26092` as CATMesh crashed the client renderer in `randy31.dll`. Mike live-validated that killing the Thief no longer crashes the client. Handbag loot interaction and the deferred mission chain remain to be validated separately.
