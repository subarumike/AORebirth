...
- Captured enemy combat now uses a shared atomic contract and runtime registry across supported and generated ordinary Subway populations. Each spawn explicitly declares fixed captured `AttackInfo`, captured equipped weapon, specialized captured behavior, or unresolved evidence; retaliation and attack-source selection use the same registry, captured weapons are equipped during spawn, contracts are removed on despawn, and incomplete contracts are logged/refused rather than becoming silent zero-damage enemies. Thief uses the captured QL1 Solar-Powered Pistol `121567`; Disobedient Bot and Lost Thought remain explicitly unresolved pending landed-hit/source evidence already absent from the completed captures.
