...
- `OfficialPlacements/AcgDevelopmentPlaceholder*`, `Tools/import_acg_development_placeholders.py`, and `docs/generated/acg_development_placeholders/`: a separate Debug-only visualization layer over the checksum-pinned portable ACG atlas. `Off` is default, Release builds reject activation, one selected playfield shard is loaded lazily, and transient entities retain explicit native-ACG visual grade and source provenance. Legacy remains the default route, its `Off` output is unchanged, and this layer has no production spawn authority. See `docs/reference/ACG_DEVELOPMENT_PLACEHOLDERS.md`.
