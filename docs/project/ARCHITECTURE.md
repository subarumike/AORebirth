...
- `docs/reference/pf4582/PlayfieldDistrictInfo.json`, `Tools/generate_pf4582_placements.py`, and `IccShuttleportPlacementCatalog*.cs`: PF4582's authoritative 206-record placement layer, deterministic normalization, explicit template/runtime evidence mapping, and fail-closed activation. `template-hash-evidence.json` and `Tools/audit_pf4582_template_hashes.py` provide the separate pinned, deterministic 38-hash identity-resolution ledger; audit results never authorize runtime activation. Placement metadata does not supply NPC behavior.
