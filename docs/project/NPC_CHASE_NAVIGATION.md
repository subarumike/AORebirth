...
PF127 tests load the promoted geometry through the generic provider, prove the representative Vergil wall blocks direct pursuit, calculate a bounded collision-valid route, follow it through the shared state machine, restore a clear direct combat path, allow the pictured body-height doorway shot while retaining movement-frame obstruction, and route a leashed NPC home through collision-valid segments. Independent regressions pass: PF127 collision/LOS `17/17` and Abmouth/Vergil `20/20`. The lifecycle assembly remains at its established `52/58` baseline with the same six unrelated session/packet ownership guardrail failures. AORebirth.Core and ZoneEngine Debug builds pass. Mike's `2026-07-17` private-client smoke confirms open-doorway attacks, real-wall blocking, chase navigation, Vergil's capture-backed `40`-unit reset, return home, and re-engagement work as intended.
