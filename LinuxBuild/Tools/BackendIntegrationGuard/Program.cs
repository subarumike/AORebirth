...
internal static class Program
{
    ...
    private static void CheckDefaults(string root)
    {
        ...
        Require(shell.Contains("engine=\"new\"") && shell.Contains(Project), "Linux default publish must select the shared New engine project.");
        Require(cmd.Contains("set ENGINE=new") && cmd.Contains(Project.Replace('/', '\\')), "Windows-hosted Linux publish default must select New.");
        ...
        Require(rollback.Contains("/ZoneEngine --headless") && rollback.Contains("Conflicts=ao-rebirth-zoneengine.service"), "Historical archived Legacy recovery must remain explicit and mutually exclusive.");
        string windows = File.ReadAllText(Path.Combine(root, "start-engines.ps1"));
        Require(windows.Contains("ZoneEngine_New"), "Windows launcher lacks the supported NewEngine runtime.");
    ...
    private static void CheckPackage(string root, string publish)
    {
        ...
        CheckPlayfieldContentPackage(root, publish);
        Require(!Directory.Exists(Path.Combine(publish, "Content/Official/PlayfieldPlacements")), "NewEngine package retains an obsolete official-placement corpus.");
    ...
    private static void CheckPlayfieldContentPackage(string root, string publish)
    {
        string source = Path.Combine(root, "AORebirth/GameData/PlayfieldContent");
        string packaged = Path.Combine(publish, "GameData/PlayfieldContent");
        Require(Directory.Exists(source), "Editable PlayfieldContent source missing.");
        Require(Directory.Exists(packaged), "Editable PlayfieldContent package missing.");

        string[] expected = Directory.GetFiles(source, "*", SearchOption.AllDirectories)
            .Select(p => Path.GetRelativePath(source, p).Replace('\\', '/'))
            .Order(StringComparer.Ordinal)
            .ToArray();
        string[] actual = Directory.GetFiles(packaged, "*", SearchOption.AllDirectories)
            .Select(p => Path.GetRelativePath(packaged, p).Replace('\\', '/'))
            .Order(StringComparer.Ordinal)
            .ToArray();
        EqualSet(expected, actual, "Editable PlayfieldContent package identity");
        Require(expected.Any(p => p.EndsWith("/Npcs.json", StringComparison.Ordinal)), "Editable playfield NPC content missing.");

        foreach (string relative in expected)
        {
            string sourceFile = Path.Combine(source, relative.Replace('/', Path.DirectorySeparatorChar));
            string packagedFile = Path.Combine(packaged, relative.Replace('/', Path.DirectorySeparatorChar));
            Require(SHA256.HashData(File.ReadAllBytes(sourceFile)).SequenceEqual(SHA256.HashData(File.ReadAllBytes(packagedFile))), "Editable PlayfieldContent hash mismatch: " + relative);
        }
        Console.WriteLine("PLAYFIELD_CONTENT_PACKAGE_PARITY=PASS");
    }
    ...
    private static void CheckContentArchitecture(string root)
    {
        string report = Path.Combine(root, "LinuxBuild/artifacts/validation/NEWENGINE_CONTENT_ARCHITECTURE_GUARD.json");
        try
        {
            Run(root, "dotnet", "run", "--project", "Tools/NewEngineRuntimeContentGuard/NewEngineRuntimeContentGuard.csproj",
                "--configuration", "Release", "--", "--write", root, "--output", report);
            Require(File.Exists(report), "Native runtime content guard did not produce its report.");
            using JsonDocument document = JsonDocument.Parse(File.ReadAllText(report));
            Require(document.RootElement.GetProperty("findings").GetArrayLength() == 0, "Native runtime retains compiled content.");
            Console.WriteLine("CONTENT_ARCHITECTURE_GUARD=PASS");
        }
        finally { /* Preserve the ignored native audit receipt with acceptance artifacts. */ }
    }
    ...
    private static string Run(string root, string file, params string[] args)
    {
        ...
        string messages = output.GetAwaiter().GetResult();
        Require(process.ExitCode == 0, "Source evaluator failed: " + error.GetAwaiter().GetResult() + messages[..Math.Min(messages.Length, 4000)]);
        return messages;
    }
