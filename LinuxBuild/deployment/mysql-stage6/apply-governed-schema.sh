...
mapfile -t expected_sql_files < <(printf '%s\n' "${SCHEMA_FILES[@]}" | LC_ALL=C sort)

[[ "${#actual_sql_files[@]}" -eq "${#expected_sql_files[@]}" ]] \
    || fail "schema directory does not contain exactly 34 SQL files"
...
echo "PASS: exact governed 34-table ChatEngine schema loaded; runtime account is restricted to row-level CRUD."
