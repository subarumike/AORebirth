...
source "${SCRIPT_DIR}/../../content-provenance.sh"
source "${SCRIPT_DIR}/package-integrity.sh"
source "${SCRIPT_DIR}/systemd-unit-state.sh"
...
while [[ "$#" -gt 0 ]]; do
    case "$1" in
        ...
        --package-preflight) package_preflight=true; shift ;;
...
[[ "${package_preflight}" == true || "${test_mode}" == "1" || "${EUID}" -eq 0 ]] || fail "run as root"
...
if [[ "${FORMAT}" == "3" ]]; then
    readonly ZONEENGINE_IMPLEMENTATION="$(manifest_value ZONEENGINE_IMPLEMENTATION)"
    readonly MANIFEST_CONTENT_PROVENANCE_VERSION="$(manifest_value CONTENT_PROVENANCE_VERSION)"
    readonly MANIFEST_CONTENT_MANIFEST_SHA="$(manifest_value CONTENT_MANIFEST_SHA256)"
    readonly MANIFEST_CONTENT_FILE_COUNT="$(manifest_value CONTENT_FILE_COUNT)"
    readonly MANIFEST_CONTENT_TOTAL_BYTES="$(manifest_value CONTENT_TOTAL_BYTES)"
elif [[ "${FORMAT}" == "2" ]]; then
...
# Deterministic manager adapter used only by the existing isolated transaction
# fixture. An unloaded unit is collected again after show, reproducing systemd GC.
if [[ "${test_mode}" == 1 ]]; then
    systemctl()
    {
        local operation="$1" service="${!#}" engine load active sub
        case "${service}" in
            ao-rebirth-loginengine.service) engine=login ;;
            ao-rebirth-zoneengine.service) engine=zone ;;
            *) return 1 ;;
        esac
        load="$(cat "${TEST_STATE}/${engine}.load")"
        active="$(cat "${TEST_STATE}/${engine}.active")"
        printf '%s %s\n' "${operation}" "${engine}" >> "${TEST_STATE}/systemd.calls"
        case "${operation}" in
            list-units)
                [[ "${load}" == unloaded || "${load}" == missing ]] || printf '%s loaded %s\n' "${service}" "${active}"
                return 0 ;;
            show)
                case "${load}" in unloaded) load=loaded ;; missing) load=not-found ;; invalid) load=bad-setting ;; esac
                case "${active}" in active) sub=running ;; inactive) sub=dead ;; failed) sub=failed ;; *) sub=unknown ;; esac
                printf 'LoadState=%s\nActiveState=%s\nSubState=%s\nUnitFileState=%s\nFragmentPath=%s\nNRestarts=%s\n' \
                    "${load}" "${active}" "${sub}" enabled "$(root_path /etc/systemd/system)/${service}" "$(cat "${TEST_STATE}/${engine}.restarts")" ;;
            reset-failed)
                if [[ "${load}" == unloaded ]]; then
                    echo "Failed to reset failed state of unit ${service}: Unit ${service} not loaded." >&2
                    return 1
                fi
                [[ "${rolling_back}" == true || "${failure_step}" != reset_failed ]] || return 1
                printf '0\n' > "${TEST_STATE}/${engine}.restarts"
                [[ "${active}" != failed ]] || printf 'inactive\n' > "${TEST_STATE}/${engine}.active"
                return 0 ;;
            *) return 1 ;;
        esac
    }
...
{
    [[ "${FORMAT}" == "2" || "${FORMAT}" == "3" ]] || fail "unsupported release manifest format"
    ...
    local unknown content_keys
    if [[ "${FORMAT}" == "3" ]]; then
        [[ "${ZONEENGINE_IMPLEMENTATION}" == new ]] || fail "editable content requires NewEngine"
        [[ "${MANIFEST_CONTENT_PROVENANCE_VERSION}" == 1 ]] || fail "unsupported content provenance version"
        [[ "${MANIFEST_CONTENT_MANIFEST_SHA}" =~ ^[0-9a-f]{64}$ ]] || fail "invalid content manifest digest"
        [[ "${MANIFEST_CONTENT_FILE_COUNT}" =~ ^[1-9][0-9]*$ ]] || fail "invalid content file count"
        [[ "${MANIFEST_CONTENT_TOTAL_BYTES}" =~ ^[1-9][0-9]*$ ]] || fail "invalid content total bytes"
        content_keys='ZONEENGINE_IMPLEMENTATION|CONTENT_PROVENANCE_VERSION|CONTENT_MANIFEST_SHA256|CONTENT_FILE_COUNT|CONTENT_TOTAL_BYTES'
    else
        content_keys='PLACEMENT_CORPUS_VERSION|PLACEMENT_CORPUS_MANIFEST_SHA256|PLACEMENT_CORPUS_SUMMARY_SHA256|PLACEMENT_CORPUS_INDEX_SHA256|PLACEMENT_ACGHASH_INVENTORY_SHA256|PLACEMENT_BUILD_MANIFEST_SHA256|PLACEMENT_RESOURCE_COUNT|PLACEMENT_PARSED_RESOURCE_COUNT|PLACEMENT_PARSER_LIMITED_RESOURCE_COUNT|PLACEMENT_DISTRICT_COUNT|PLACEMENT_RECORD_COUNT|PLACEMENT_UNIQUE_ACGHASH_COUNT|PLACEMENT_RUNTIME_AUTHORIZED_COUNT'
    fi
    unknown="$(cut -d= -f1 "${manifest_path}" | grep -Ev "^(FORMAT|SOURCE_SHA|BUILD_TIMESTAMP_UTC|LOGINENGINE_ARTIFACT_DIR|LOGINENGINE_ARTIFACT_SHA256|ZONEENGINE_ARTIFACT_DIR|ZONEENGINE_ARTIFACT_SHA256|${content_keys}|LOGINENGINE_UNIT_PATH|LOGINENGINE_UNIT_SHA256|ZONEENGINE_UNIT_PATH|ZONEENGINE_UNIT_SHA256|LOGINENGINE_SERVICE|ZONEENGINE_SERVICE|PREVIOUS_LOGINENGINE_RELEASE|PREVIOUS_ZONEENGINE_RELEASE)$" | head -n 1 || true)"
    [[ -z "${unknown}" ]] || fail "unknown manifest key ${unknown}"
}

# Content hashes prove accepted artifact identity; they do not authorize NPCs or game content.
require_zone_content_artifact()
{
    local artifact_dir="$1"
    if [[ "${FORMAT}" == "2" ]]; then
        require_zone_placement_artifact "${artifact_dir}"
        return
    fi
    content_provenance_load "${artifact_dir}" "${SOURCE_SHA}" linux "${MANIFEST_CONTENT_MANIFEST_SHA}" \
        || fail "ZoneEngine editable content manifest is invalid"
    content_require_build_provenance "${artifact_dir}/BUILD_PROVENANCE.env" \
        || fail "ZoneEngine build provenance lacks editable content integrity"
    [[ "${CONTENT_PROVENANCE_VERSION}" == "${MANIFEST_CONTENT_PROVENANCE_VERSION}" \
        && "${CONTENT_FILE_COUNT}" == "${MANIFEST_CONTENT_FILE_COUNT}" \
        && "${CONTENT_TOTAL_BYTES}" == "${MANIFEST_CONTENT_TOTAL_BYTES}" ]] \
        || fail "ZoneEngine content manifest metadata mismatch"
    local accepted_line
    for accepted_line in "CONTENT_VALIDATION=PASS" "CONTENT_ARCHITECTURE_GUARD=PASS" \
        "CONTENT_MANIFEST_SHA256=${MANIFEST_CONTENT_MANIFEST_SHA}"; do
        grep -Fx "${accepted_line}" "${artifact_dir}/LINUX_ACCEPTANCE.env" >/dev/null \
            || fail "ZoneEngine editable content acceptance is missing or mismatched"
    done
}
...
{
    systemd_unit_prepare_start "$2" "$(root_path /etc/systemd/system)/$2"
}
service_stop()
{
    if [[ "${test_mode}" == "1" ]]; then
...
{
    local engine="$1"
    service_reset_failed "$1" "$2" || return 1
    if [[ "${test_mode}" == "1" ]]; then
        [[ "${rolling_back}" == "true" || "${failure_step}" != "${engine}_start" ]] || return 1
        printf 'active\n' > "${TEST_STATE}/${engine}.active"
        printf 'loaded\n' > "${TEST_STATE}/${engine}.load"
...
{
    if [[ "${test_mode}" == "1" ]]; then
        [[ "${rolling_back}" == true || "${failure_step}" != daemon_reload ]] || return 1
        if [[ "$(cat "${TEST_STATE}/unload-on-reload")" == YES ]]; then
            for engine in login zone; do
                [[ "$(cat "${TEST_STATE}/${engine}.active")" != inactive ]] || printf 'unloaded\n' > "${TEST_STATE}/${engine}.load"
            done
        fi
...
verify_unit_static()
(
    [[ "${test_mode}" != 1 || "${mutation_started}" != true || "${rolling_back}" == true || "${failure_step}" != unit_validation ]] || exit 1
    [[ "${test_mode}" != "1" ]] || exit 0
...
preflight()
{
    ...
    require_zone_content_artifact "${ZONE_ARTIFACT_DIR}"
    validate_units
    verify_unit_static
    validate_zone_dropin_preflight
    systemd_unit_inspect "${LOGIN_SERVICE}" "${LOGIN_UNIT_TARGET}" || fail "LoginEngine systemd preflight failed"
    systemd_unit_inspect "${ZONE_SERVICE}" "${ZONE_UNIT_TARGET}" || fail "ZoneEngine systemd preflight failed"
    echo "SYSTEMD_UNIT_PREFLIGHT=PASS"
...
current_release_matches()
{
    ...
    local current_zone_release
    current_zone_release="$(realpath -e -- "${ZONE_CURRENT}")" || return 1
    ( require_zone_content_artifact "${current_zone_release}" ) >/dev/null 2>&1 || return 1
...
install_release()
{
    ...
    if [[ -e "${target}" ]]; then
        ...
        if [[ "${apphost}" == "${ZONE_APPHOST}" ]]; then
            require_zone_content_artifact "${target}"
        fi
        return
    fi
    ...
    if [[ "${apphost}" == "${ZONE_APPHOST}" ]]; then
        require_zone_content_artifact "${staging}"
    fi
    mv -T -- "${staging}" "${target}"
}
...
FORMAT=${FORMAT}
EOF
    grep -E '^(PLACEMENT_|CONTENT_|ZONEENGINE_IMPLEMENTATION=)' "${manifest_path}" >> "${temporary}"
...
post_health()
{
    service_active login "${LOGIN_SERVICE}" && service_active zone "${ZONE_SERVICE}" \
        ...
        && zone_effective_notify_contract \
        && [[ -n "${LOGIN_RESTARTS_START_BASELINE}" ]] \
        && [[ "$(service_restarts login "${LOGIN_SERVICE}")" == "${LOGIN_RESTARTS_START_BASELINE}" ]] \
...
main()
{
    ...
    if [[ -f "${package_root}/LinuxBuild/PACKAGE_CONTENTS.tsv" || "${package_preflight}" == true ]] \
        || [[ "${test_mode}" != 1 && ! -e "${package_root}/.git" ]]; then
        package_verify "${package_root}" || fail "release package integrity failed"
    fi
    if [[ "${package_preflight}" == true ]]; then
        validate_manifest_shape
        [[ "${LOGIN_ARTIFACT_DIR}" == "${package_root}/LinuxBuild/artifacts/loginengine/linux-x64/self-contained" \
            && "${ZONE_ARTIFACT_DIR}" == "${package_root}/LinuxBuild/artifacts/zoneengine/linux-x64/self-contained" ]] \
            || fail "package manifest is bound to a different extraction directory"
        require_artifact "${LOGIN_ARTIFACT_DIR}" LoginEngine "${LOGIN_ARTIFACT_SHA}"
        require_artifact "${ZONE_ARTIFACT_DIR}" "${ZONE_APPHOST}" "${ZONE_ARTIFACT_SHA}"
        require_zone_content_artifact "${ZONE_ARTIFACT_DIR}"
        echo "PLACEMENT_PROVENANCE_PREFLIGHT=PASS"
        echo "PACKAGE_PREFLIGHT=PASS"
    ...
    if [[ -f "${package_root}/LinuxBuild/PACKAGE_CONTENTS.tsv" ]]; then
        package_verify_installed "${package_root}" "${LOGIN_RELEASE_TARGET}" "${ZONE_RELEASE_TARGET}" \
            || fail "installed release identity failed before startup"
    fi
    service_reset_failed login "${LOGIN_SERVICE}"
    LOGIN_RESTARTS_START_BASELINE="$(service_restarts login "${LOGIN_SERVICE}")"
