...
tests_run=0
source "${repository_root}/LinuxBuild/content-provenance.sh"
...
create_placement_artifact()
{
    ...
    if [[ "${fixture_content_only:-0}" == 1 ]]; then
        ...
        fixture_placement_build_manifest_sha=''
        return
    fi
...
# NewEngine content fixtures contain no Official placement assets or fixed corpus counts.
create_content_fixture()
{
    fixture_content_only=1
    create_fixture
    fixture_content_only=0
    mkdir -p "${input}/zone/GameData/PlayfieldContent/127" "${input}/zone/Content/Runtime"
    printf '{"SchemaVersion":1,"PlayfieldId":127,"Npcs":[],"StandaloneShops":[]}\n' > "${input}/zone/GameData/PlayfieldContent/127/Npcs.json"
    printf '{"Actions":[]}\n' > "${input}/zone/Content/Runtime/interactions.json"
    content_provenance_write "${input}/zone" "${fake_sha}" linux
    content_append_build_provenance "${input}/zone/BUILD_PROVENANCE.env"
    cat >> "${input}/zone/LINUX_ACCEPTANCE.env" <<EOF
CONTENT_VALIDATION=PASS
CONTENT_ARCHITECTURE_GUARD=PASS
CONTENT_MANIFEST_SHA256=${CONTENT_MANIFEST_SHA256}
EOF
    sed -i '/^PLACEMENT_/d' "${manifest}"
    set_manifest_value "${manifest}" FORMAT 3
    cat >> "${manifest}" <<EOF
ZONEENGINE_IMPLEMENTATION=new
...
EOF
    require test ! -e "${input}/zone/Content/Official"
}

run_editable_content_cases()
{
    local initial_count="${tests_run}"
    create_content_fixture
    run_upgrade > "${fixture}/output"
    require grep -F 'TRANSACTIONAL_DEPLOYMENT=PASS' "${fixture}/output"
    require test -f "${root}/opt/ao-rebirth/zoneengine/current/CONTENT_MANIFEST.json"
    tests_run=$((tests_run + 1))
    ...
    expect_preflight_failure
    create_content_fixture
    rm -f -- "${input}/zone/CONTENT_MANIFEST.json"
    expect_preflight_failure
    create_content_fixture
    set_manifest_value "${manifest}" CONTENT_MANIFEST_SHA256 "$(printf bad | sha256sum | awk '{print $1}')"
    expect_preflight_failure
    create_content_fixture
    set_manifest_value "${manifest}" CONTENT_FILE_COUNT 999
    expect_preflight_failure
    create_content_fixture
    sed -i '/^CONTENT_ARCHITECTURE_GUARD=PASS$/d' "${input}/zone/LINUX_ACCEPTANCE.env"
    expect_preflight_failure
    create_content_fixture
    printf 'PLACEMENT_RUNTIME_AUTHORIZED_COUNT=199\n' >> "${manifest}"
    expect_preflight_failure
    create_content_fixture
    set_manifest_value "${manifest}" ZONEENGINE_IMPLEMENTATION legacy
    expect_preflight_failure
    create_content_fixture
    printf '[]\n' > "${input}/zone/GameData/uninventoried.json"
    expect_preflight_failure
    create_content_fixture
    expect_transaction_failure unit_install
    require test "${tests_run}" = "$((initial_count + 11))"
    echo 'EDITABLE_CONTENT_DEPLOYMENT_TESTS=PASS cases=11'
}

case "${AO_REBIRTH_DEPLOY_TEST_CASE:-all}" in
    all) ;;
    editable-content) run_editable_content_cases; exit 0 ;;
    success)
        create_fixture
...
behavior_hash_before="$(sha256sum "${repository_root}/AORebirth/Server/LoginEngine/CoreClient/LoginHandoffLifecycle.cs" \
    "${repository_root}/AORebirth/Server/ZoneEngine_New/Program.cs" \
    "${repository_root}/AORebirth/Server/ZoneEngine_New/Core/GameData/GameDataStore.cs")"
...
require grep -F '<ZoneIP>192.0.2.30</ZoneIP>' "$(cat "${state}/candidate-login-config")"
require grep -F '<ZoneIP>192.0.2.30</ZoneIP>' "$(cat "${state}/candidate-zone-config")"
...
# Model the production stop/install/reload/GC boundary, including automatic
# rollback starts. Every failure must still restore the exact previous pair.
create_fixture
printf 'YES\n' > "${state}/unload-on-reload"
run_upgrade > "${fixture}/output"
require grep -F 'class=UNIT_ACTIVE' "${fixture}/output"
require grep -F 'class=UNIT_NOT_LOADED_BUT_UNIT_FILE_EXISTS' "${fixture}/output"
require grep -F 'TRANSACTIONAL_DEPLOYMENT=PASS' "${fixture}/output"
if grep -Fq reset-failed "${state}/systemd.calls"; then fail 'reset attempted for unloaded valid zero-restart unit'; fi
echo 'UPGRADE_UNLOADED_UNIT_PATH=PASS'
echo 'UNLOADED_VALID_UNIT_HANDLED=PASS'
tests_run=$((tests_run + 1))

for step in artifact_install unit_install daemon_reload unit_validation login_start zone_start listener; do
    create_fixture
    printf 'YES\n' > "${state}/unload-on-reload"
    expect_transaction_failure "${step}"
    require grep -F 'class=UNIT_NOT_LOADED_BUT_UNIT_FILE_EXISTS' "${fixture}/output"
done
echo 'ROLLBACK_UNLOADED_UNIT_PATH=PASS'
echo 'DEPLOYMENT_TRANSACTION_FAIL_CLOSED=PASS'

for invalid in missing masked invalid activating; do
    create_fixture
    if [[ "${invalid}" == activating ]]; then
        printf 'activating\n' > "${state}/zone.active"
    else
        printf '%s\n' "${invalid}" > "${state}/zone.load"
    fi
    if run_upgrade > "${fixture}/output" 2>&1; then fail "accepted invalid unit ${invalid}"; fi
    assert_old_targets
    require test "$(cat "${state}/login.starts")" = 0
    tests_run=$((tests_run + 1))
done
create_fixture
rm -- "${root}/etc/systemd/system/ao-rebirth-zoneengine.service"
if run_upgrade > "${fixture}/output" 2>&1; then fail 'accepted absent unit file'; fi
require test "$(cat "${state}/login.starts")" = 0
tests_run=$((tests_run + 1))
...
set_stopped_pair
run_stopped_recovery_upgrade > "${fixture}/output"
require grep -F 'class=UNIT_INACTIVE_LOADED' "${fixture}/output"
tests_run=$((tests_run + 1))
...
printf '3\n' > "${state}/zone.restarts"
run_recovery_upgrade > "${fixture}/output"
require grep -F 'class=UNIT_FAILED_LOADED' "${fixture}/output"
require grep -F 'SYSTEMD_RESET_DECISION=RESET_FAILURE_OR_COUNTER' "${fixture}/output"
require test "$(cat "${state}/zone.restarts")" = 0
tests_run=$((tests_run + 1))

create_fixture
printf '3\n' > "${state}/zone.restarts"
expect_transaction_failure reset_failed
require grep -F 'reset failed for ao-rebirth-zoneengine.service' "${fixture}/output"

create_fixture
printf '2\n' > "${state}/login.restarts"
printf '3\n' > "${state}/zone.restarts"
run_upgrade > "${fixture}/output"
require grep -F 'TRANSACTIONAL_DEPLOYMENT=PASS' "${fixture}/output"
require test "$(cat "${state}/login.restarts")" = 0
require test "$(cat "${state}/zone.restarts")" = 0
tests_run=$((tests_run + 1))

require test "${tests_run}" = 81
echo "PASS: production deployment workflow tests (81/81)"
