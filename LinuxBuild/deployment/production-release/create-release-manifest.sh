...
content_provenance_load "${zone_artifact_dir}" "${expected_sha}" linux \
    || fail "ZoneEngine editable content manifest is invalid"
content_require_build_provenance "${zone_artifact_dir}/BUILD_PROVENANCE.env" \
...
for accepted_line in "CONTENT_VALIDATION=PASS" "CONTENT_ARCHITECTURE_GUARD=PASS" \
    "CONTENT_MANIFEST_SHA256=${CONTENT_MANIFEST_SHA256}"; do
    grep -Fx "${accepted_line}" "${zone_artifact_dir}/LINUX_ACCEPTANCE.env" >/dev/null \
