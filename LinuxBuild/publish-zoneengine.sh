...
[[ "$#" -le 2 ]] || { echo "Engine selection is retired; NewEngine is the only publish target." >&2; exit 2; }
engine="new"
project="../AORebirth/Server/ZoneEngine_New/ZoneEngine_New.csproj"
artifact_name="zoneengine"
case "${runtime_id}" in
...
content_provenance_write "${publish_dir}" "${source_sha}" linux
