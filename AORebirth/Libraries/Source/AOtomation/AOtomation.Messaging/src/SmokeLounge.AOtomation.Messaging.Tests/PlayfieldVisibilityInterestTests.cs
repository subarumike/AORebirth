namespace SmokeLounge.AOtomation.Messaging.Tests
{
    ...
    public class PlayfieldVisibilityInterestTests
    {
        ...
        public void AllCapturedPf127CatalogRowsResolveProfilesAndEnterSharedSpatialIndex()
        {
            ...
            Assert.AreEqual(259, spawns.Length);
            Assert.AreEqual(259, spawns.Select(value => value.SourceIdentity).Distinct().Count());
            Assert.AreEqual(259, spawns.Count(value => value.PlayfieldInstance == 127));
            ...
            Assert.AreEqual(259, index.Count);
            Assert.AreEqual(
                221,
                spawns.Count(
            ...
            Assert.AreEqual(17, profiles.Length);
            foreach (AORebirth.Core.Playfields.OrdinaryEnemySpawnDefinition spawn in spawns)
            {
                Assert.AreEqual(
