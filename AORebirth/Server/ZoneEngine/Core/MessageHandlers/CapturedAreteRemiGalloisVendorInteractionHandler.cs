namespace ZoneEngine.Core.MessageHandlers
{
    ...
    /// <summary>
    /// Capture 20260727-213512: GenericCmd Use Remi (knubot shop cart)
    /// → ShopUpdate + Trade Open.
    /// </summary>
    internal sealed class CapturedAreteRemiGalloisVendorInteractionHandler
    {
        internal static readonly CapturedAreteRemiGalloisVendorInteractionHandler Default =
            new CapturedAreteRemiGalloisVendorInteractionHandler();

        private CapturedAreteRemiGalloisVendorInteractionHandler()
        {
        }
        ...
        internal bool TryHandleUse(IZoneClient client, GenericCmdMessage message, Identity target)
        {
            ...
            {
                CapturedAreteRemiGalloisVendorRuntimeDefinition runtime;
                if (!CapturedAreteRemiGalloisVendorRuntimeRegistry.TryGet(target.Instance, out runtime))
                {
                    return false;
                }
        ...
        internal bool TryOpenShop(ICharacter character, Identity npcIdentity)
        {
            ...
            // Capture: knubot Shop cart / Use Remi (SimpleChar) only.
            ...
            CapturedAreteRemiGalloisVendorRuntimeDefinition runtime;
            if (!CapturedAreteRemiGalloisVendorRuntimeRegistry.TryGet(npcIdentity.Instance, out runtime))
            {
                return false;
            }

            if (!CapturedAreteRemiGalloisVendorRuntimeRegistry.Same(runtime.NpcIdentity, npcIdentity))
            {
                return false;
            }

            if (!CapturedAreteRemiGalloisVendorRuntimeRegistry.Same(
            ...
            LogUtil.Debug(
                ...
                "Remi Gallois shop open character="
                + character.Identity
                + " vendor="
                + vendor.Identity
                + " slots="
