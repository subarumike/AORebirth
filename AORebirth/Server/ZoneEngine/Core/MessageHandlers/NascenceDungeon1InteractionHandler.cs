namespace ZoneEngine.Core.MessageHandlers
{
    ...
    /// <summary>
    /// Capture 20260823-171238: interior door Use (ACK Temp1=2 + DoorStatus open) and
    /// floor Button (down/up/boss) local teleports.
    /// </summary>
    public sealed class NascenceDungeon1InteractionHandler
    {
        public static readonly NascenceDungeon1InteractionHandler Default =
            new NascenceDungeon1InteractionHandler();

        private const int ButtonDown = unchecked((int)0x57EC93CE);

        private const int ButtonUpLower = unchecked((int)0x57EC93CF);

        private const int ButtonBoss = unchecked((int)0x57EC93D2);

        private const int ButtonUpBoss = unchecked((int)0x57EC93D3);

        private NascenceDungeon1InteractionHandler()
        {
        }
        ...
        public bool TryHandleUse(IZoneClient client, GenericCmdMessage message, Identity target)
        {
            ...
            if (target.Type == IdentityType.Door
                && NascenceDungeon1DoorCapture.IsInteriorDoorInstance(target.Instance))
            {
                ...
                LogUtil.Debug(
                    ...
                    "NascenceDungeon1 door Use ACK Temp1=2 door=" + target.Instance.ToString("X8"));
                return true;
            }
            ...
            if (zoneClient != null)
            {
                NascenceDungeon1DoorReplay.RefreshFloorButtonsAfterTeleport(
                    zoneClient,
                    character,
                    target.Instance);
                NascenceDungeon1DoorReplay.RevealZoneAtPosition(zoneClient, character, destX, destZ);
                if (IsBossDownButton(target.Instance))
                {
                    NascenceDungeon1DoorReplay.RevealBossWingForCharacter(zoneClient, character);
                    var playfieldForBoss = character.Playfield as Playfield;
                    if (playfieldForBoss != null)
                    {
                        NascenceDungeon1BossRoomRuntime.ForceHavarisVisible(playfieldForBoss, character);
                    }
                }
            ...
            LogUtil.Debug(
                ...
                string.Format(
                    ...
                    "NascenceDungeon1 button Use terminal={0:X8} dest=({1:F3},{2:F3},{3:F3})",
                    target.Instance,
                    destX,
                    destY,
                    destZ));
