namespace ZoneEngine.Core.MessageHandlers
{
    ...
    /// <summary>
    /// Capture 20260801-burger-vendor: GenericCmd Use Barry (knubot shop cart)
    /// → ShopUpdate + Trade Open.
    /// </summary>
    internal sealed class CapturedAreteBarryFoodVendorInteractionHandler
    {
        internal static readonly CapturedAreteBarryFoodVendorInteractionHandler Default =
            new CapturedAreteBarryFoodVendorInteractionHandler();

        private CapturedAreteBarryFoodVendorInteractionHandler()
        {
        }
        ...
        internal bool TryHandleUse(IZoneClient client, GenericCmdMessage message, Identity target)
        {
            ...
            {
                CapturedAreteBarryFoodVendorRuntimeDefinition runtime;
                if (!CapturedAreteBarryFoodVendorRuntimeRegistry.TryGet(target.Instance, out runtime))
                {
                    return false;
                }
        ...
        internal bool TryOpenShop(ICharacter character, Identity npcIdentity)
        {
            ...
            // Capture: knubot Shop cart / Use Barry (SimpleChar) only.
            ...
            CapturedAreteBarryFoodVendorRuntimeDefinition runtime;
            if (!CapturedAreteBarryFoodVendorRuntimeRegistry.TryGet(npcIdentity.Instance, out runtime))
            {
                return false;
            }

            if (!CapturedAreteBarryFoodVendorRuntimeRegistry.Same(runtime.NpcIdentity, npcIdentity))
            {
                return false;
            }

            if (!CapturedAreteBarryFoodVendorRuntimeRegistry.Same(
            ...
            LogUtil.Debug(
                ...
                "Barry food shop open character="
                + character.Identity
                + " vendor="
                + vendor.Identity
                + " slots="
