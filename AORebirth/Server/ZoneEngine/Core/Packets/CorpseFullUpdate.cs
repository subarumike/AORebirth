namespace ZoneEngine.Core.Packets
{
    ...
    public static class CorpseFullUpdate
    {
        ...
        private static byte[] BuildCapturedMissionTrashCorpse(
            ...
            int corpseCredits)
        {
            return GeneratedMissionCorpseWire.Build(
                deadNpc.Name, deadNpc.Identity.Instance, corpseIdentity.Instance,
                receiver.Instance, serverId,
                (float)deadNpc.Position.x, (float)deadNpc.Position.y, (float)deadNpc.Position.z,
                deadNpc.Playfield.Identity.Instance, deadNpc.Stats[StatIds.monsterscale].Value,
                deadNpc.Stats[StatIds.sex].Value, deadNpc.Stats[StatIds.breed].Value,
                deadNpc.Stats[StatIds.race].Value, corpseCatMesh, corpseMonsterData, corpseCredits);
        }
