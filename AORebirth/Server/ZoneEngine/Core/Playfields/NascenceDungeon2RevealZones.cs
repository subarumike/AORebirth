namespace AORebirth.Core.Playfields
{
    ...
    /// <summary>
    /// ACG room reveal for Nascence D2 — props are visible for the whole grid cell (zone),
    /// not by proximity radius. Capture 20260823-182854 prop positions, 96m cells.
    /// </summary>
    internal static class NascenceDungeon2RevealZones
    {
        ...
        internal static void EnsureBuilt()
        {
            ...
            lock (Gate)
            {
                ...
                IndexPackets(NascenceDungeon2DoorCapture.ZoneInDoorPacketHex, doorsByZone);
                IndexPackets(NascenceDungeon2DoorCapture.ZoneInTerminalPacketHex, terminalsByZone);
                IndexPackets(NascenceDungeon2DoorCapture.ZoneInChestPacketHex, chestsByZone);
                built = true;
                ...
                Utility.LogUtil.Debug(
                    ...
                    string.Format(
                        ...
                        "NascenceDungeon2RevealZones indexed doors={0} terminals={1} chests={2} doorZones={3} terminalZones={4} chestZones={5}",
            ...
            HashSet<long> keys)
        {
            foreach (KeyValuePair<long, List<ZonePacket>> entry in map)
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    if (list[i].X < NascenceDungeon2Rules.BossWingMaxWorldX)
                    {
                        keys.Add(entry.Key);
                        break;
                    }
