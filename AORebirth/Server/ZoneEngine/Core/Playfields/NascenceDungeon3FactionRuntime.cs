namespace AORebirth.Core.Playfields
{
    ...
    /// <summary>
    /// Capture 20260830-140240: GOS (Guardian of Shadow) rises only after
    /// Mortiig Predator kills (NascenceDungeon3Rules.GrantsGuardianOfShadowFaction).
    /// FormatFeedback #gKrfR Li pair.
    /// </summary>
    internal static class NascenceDungeon3FactionRuntime
    {
        ...
        internal static bool TryApplyKillFactionGain(ICharacter attacker, ICharacter victim)
        {
            ...
            if (attacker.Playfield == null
                || !NascenceDungeon3Rules.IsDungeonPlayfield(attacker.Playfield.Identity.Instance))
            {
                return false;
            }

            if (!NascenceDungeon3Rules.GrantsGuardianOfShadowFaction(victim.Name))
            {
                return false;
            }
            ...
            try
            {
                ...
                attacker.Stats[StatIds.gos].Value =
                    current + NascenceDungeon3Rules.GuardianOfShadowFactionGainPerKill;
            }
            catch (Exception)
            {
                return false;
