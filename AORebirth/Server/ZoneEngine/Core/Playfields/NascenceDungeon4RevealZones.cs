namespace AORebirth.Core.Playfields
{
    ...
    /// <summary>
    /// ACG room reveal for Nascence D4 — props are visible for the whole grid cell (zone),
    /// not by proximity radius. Capture 20260830-143801 prop positions from NascenceDungeon4DoorCapture, 48m cells.
    /// </summary>
    internal static class NascenceDungeon4RevealZones
    {
        ...
        internal static void EnsureBuilt()
        {
            ...
            lock (Gate)
            {
                ...
                IndexPackets(NascenceDungeon4DoorCapture.ZoneInDoorPacketHex, doorsByZone);
                IndexPackets(NascenceDungeon4DoorCapture.ZoneInTerminalPacketHex, terminalsByZone);
                IndexPackets(NascenceDungeon4DoorCapture.ZoneInChestPacketHex, chestsByZone);
                built = true;
                ...
                Utility.LogUtil.Debug(
                    ...
                    string.Format(
                        ...
                        "NascenceDungeon4RevealZones indexed doors={0} terminals={1} chests={2} doorZones={3} terminalZones={4} chestZones={5}",
            ...
            HashSet<long> keys)
        {
            foreach (KeyValuePair<long, List<ZonePacket>> entry in map)
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    if (list[i].X < NascenceDungeon4Rules.BossWingMaxWorldX)
                    {
                        keys.Add(entry.Key);
                        break;
                    }
        ...
        internal static bool TryParseWorldPosition(byte[] packet, out float x, out float y, out float z)
        {
            ...
            for (int i = 0; i + 28 <= packet.Length; i++)
            {
                ...
                // Capture 20260830-143801: D4 entry wing doors/chests sit at X≈1498–1500
                // (landing InteriorLandingX=1498.2). Widen past D3's x<1500 so edge props index.
                // Floor buttons also use Y≈64–76.
                if (y > 20f && y < 90f && x > 50f && x < 1600f && z > 0f && z < 400f)
                {
                    return true;
                }
            }
