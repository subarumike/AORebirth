namespace AORebirth.Core.Playfields
{
    ...
    /// <summary>
    /// ACG room reveal for Nascence D1 — props are visible for the whole grid cell (zone),
    /// not by proximity radius. Capture 20260823-171238 prop positions, 96m cells.
    /// </summary>
    internal static class NascenceDungeon1RevealZones
    {
        ...
        internal static void EnsureBuilt()
        {
            ...
            lock (Gate)
            {
                ...
                IndexPackets(NascenceDungeon1DoorCapture.ZoneInDoorPacketHex, doorsByZone);
                IndexPackets(NascenceDungeon1DoorCapture.ZoneInTerminalPacketHex, terminalsByZone);
                IndexPackets(NascenceDungeon1DoorCapture.ZoneInChestPacketHex, chestsByZone);
                built = true;
                ...
                Utility.LogUtil.Debug(
                    ...
                    string.Format(
                        ...
                        "NascenceDungeon1RevealZones indexed doors={0} terminals={1} chests={2} doorZones={3} terminalZones={4} chestZones={5}",
            ...
            HashSet<long> keys)
        {
            foreach (KeyValuePair<long, List<ZonePacket>> entry in map)
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    if (list[i].X < NascenceDungeon1Rules.BossWingMaxWorldX)
                    {
                        keys.Add(entry.Key);
                        break;
                    }
        ...
        internal static bool TryParseWorldPosition(byte[] packet, out float x, out float y, out float z)
        {
            ...
            for (int i = 0; i + 28 <= packet.Length; i++)
            {
                ...
                if (y > 40f && y < 80f && x > 50f && x < 1100f && z > 0f && z < 400f)
                {
                    return true;
                }
            }
