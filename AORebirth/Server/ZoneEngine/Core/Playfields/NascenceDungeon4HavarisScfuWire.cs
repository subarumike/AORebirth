namespace ZoneEngine.Core.Playfields
{
    ...
    /// <summary>
    /// Capture 20260830-143801 Havaris SCFU wire (reuses D1/D2 body template with D3 PF gate).
    ...
    /// </summary>
    internal static class NascenceDungeon4HavarisScfuWire
    {
        ...
        private const int ScfuHealthDamageOffset = 95;

        // Capture RawPacketHex (seq placeholder 0000 + body). Length field 0x0149 includes seq word.
        // ExtTex "[2] opacity maps" must use 0x20 space (not 0x30) or textures fail and boss is invisible.
        ...
        internal static byte[] BuildPacket(Character havaris, int recipientInstance)
        {
            ...
            else if (maxHealth > ushort.MaxValue)
            {
            ...
            // Template flags HasSmallHealthDamage → HealthDamage is one byte (not ushort).
            int healthDamage = maxHealth - currentHealth;
            if (healthDamage < 0)
            {
                healthDamage = 0;
            }
            else if (healthDamage > byte.MaxValue)
            {
                healthDamage = byte.MaxValue;
            }
            ...
            // Health = max life; HealthDamage = max - current (never write current into Health).
            WriteUInt16BigEndian(packet, ScfuHealthOffset, (ushort)maxHealth);
            packet[ScfuHealthDamageOffset] = (byte)healthDamage;
