namespace AORebirth.Core.Playfields
{
    ...
    /// <summary>
    /// Capture 20260830-143801: Treasure (C749) Use opens container UI + loot — not credits+despawn.
    /// High-confidence per-chest: 0BAE7776 → Cracked Crystal 220519 QL70 (chat + InventoryUpdate).
    /// Capture 20260830-143801: 37 chest opens; most InventoryUpdate rows carry trash loot,
    /// only a few are empty (x3f1=0x3F1). Until full per-chest decode lands, uncaptured
    /// chests use capture-family FallbackLootPools (same trash tables as D3 ACG treasure).
    /// Inventory observations: Container bag + handle 114+, flags 0x21, GenericCmd ACK Temp1=1.
    /// </summary>
    internal static class NascenceDungeon4TreasureLootService
    {
        ...
        private static readonly TimeSpan RespawnDelay = NascenceDungeon4Rules.TreasureRespawnDelay;
        ...
        internal static void ProcessDue(Playfield playfield, DateTime utcNow)
        {
            ...
            for (int i = 0; i < due.Count; i++)
            {
                ...
                NascenceDungeon4DoorReplay.RespawnTreasureChestInZone(
                    playfield,
                    pending.ZoneKey,
                ...
                LogUtil.Debug(
                    ...
                    string.Format(
                        ...
                        "NascenceDungeon4 treasure respawn chest={0:X8} zone={1:X16} pf={2}",
                        pending.ContainerIdentity.Instance,
                        pending.ZoneKey,
        ...
        /// <summary>
        /// True when this character currently has the chest loot UI open — do not re-send closed CFU.
        /// </summary>
        internal static bool IsLootUiOpenFor(int characterInstance, int containerInstance)
        {
            if (characterInstance == 0 || containerInstance == 0)
            {
                return false;
            }
            ...
            lock (Sync)
            {
                ...
                if (!Chests.TryGetValue(Key(new Identity
                {
                    Type = IdentityType.Container,
                ...
                return chest.Opened && chest.OpenedByCharacterInstance == characterInstance;
            }
        }
        ...
        internal static bool TryHandleUse(IZoneClient client, GenericCmdMessage message, Identity target)
        {
            ...
            if (chest.Opened && chest.OpenedByCharacterInstance == character.Identity.Instance)
            {
                ...
                if (playfield != null)
                {
                    TryDespawnIfFullyEmptied(playfield, chest);
                }
            ...
            if (!chest.TrapSpawned)
            {
                chest.TrapSpawned = true;
                var playfield = character.Playfield as Playfield;
                if (playfield != null)
                {
                    NascenceDungeon4Spawn.TrySpawnChestTrap(
                        playfield,
                        chest.ContainerIdentity.Instance);
                }
            }

            // Use leaves the client attack-busy until UseActionFinished (Mike: Wait for finish).
            SendUseActionFinished(character);
            ...
            LogUtil.Debug(
                ...
                string.Format(
                    System.Globalization.CultureInfo.InvariantCulture,
                    "NascenceDungeon4 treasure open chest={0:X8} handle={1} items={2} char={3}",
                    chest.ContainerIdentity.Instance,
            ...
            int targetPlacement)
        {
            ...
            if (!TryResolveLootRequest(sourceContainer, out chest, out lootSlot))
            {
                LogUtil.Debug(
                    ...
                    string.Format(
                        System.Globalization.CultureInfo.InvariantCulture,
                        "NascenceDungeon4 treasure loot unresolved source={0} type={1} instance={2:X8}",
                        sourceContainer,
            ...
            lock (Sync)
            {
                ...
                if (slot == null)
                {
                    LogUtil.Debug(
                        ...
                        string.Format(
                            System.Globalization.CultureInfo.InvariantCulture,
                            "NascenceDungeon4 treasure loot miss-slot chest={0:X8} handle={1} slot={2} source={3}",
                            chest.ContainerIdentity.Instance,
                            chest.InventoryHandle,
                            lootSlot,
                            sourceContainer));
            ...
            if (!TryCreateLootItem(slot, out item))
            {
                LogUtil.Debug(
                    ...
                    string.Format(
                        System.Globalization.CultureInfo.InvariantCulture,
                        "NascenceDungeon4 treasure loot bad-template chest={0:X8} low={1} high={2} ql={3}",
                        chest.ContainerIdentity.Instance,
                        slot.LowId,
                        slot.HighId,
                        slot.Quality));
            ...
            if (transfer.Status != CorpseLootInventoryTransferStatus.Success)
            {
                LogUtil.Debug(
                    ...
                    string.Format(
                        System.Globalization.CultureInfo.InvariantCulture,
                        "NascenceDungeon4 treasure loot transfer-fail chest={0:X8} status={1} invErr={2} ex={3}",
                        chest.ContainerIdentity.Instance,
                        transfer.Status,
            ...
            LogUtil.Debug(
                ...
                string.Format(
                    System.Globalization.CultureInfo.InvariantCulture,
                    "NascenceDungeon4 treasure loot ok chest={0:X8} low={1} high={2} invSlot={3}",
                    chest.ContainerIdentity.Instance,
                    item.LowID,
                    item.HighID,
                    transfer.TargetSlot));
        ...
        private static bool HasUnlootedItems(TreasureChestState chest)
        {
            return chest != null && chest.Items != null && chest.Items.Any(x => !x.Looted);
        }

        private static void TryDespawnIfFullyEmptied(Playfield playfield, TreasureChestState chest)
        {
            if (playfield == null || chest == null || HasUnlootedItems(chest))
            {
                return;
            }
            ...
            long zoneKey = NascenceDungeon4RevealZones.ZoneKeyForContainer(chest.ContainerIdentity.Instance);
            lock (Sync)
            {
            ...
            LogUtil.Debug(
                ...
                string.Format(
                    System.Globalization.CultureInfo.InvariantCulture,
                    "NascenceDungeon4 treasure despawn-empty chest={0:X8} zone={1:X16}",
                    chest.ContainerIdentity.Instance,
                    zoneKey));
        }
        ...
        private static string ResolveLootItemDisplayName(Item item)
        {
            ...
            // Capture 20260830-143801 chat name when ItemNames row is missing.
        ...
        private static bool TryCreateLootItem(TreasureLootSlot slot, out Item item)
        {
            ...
            catch (Exception ex)
            {
                LogUtil.Debug(
                    DebugInfoDetail.Error,
                    "NascenceDungeon4 treasure Item create failed id="
                    + templateId
                    + " err="
                    + ex.Message);
                item = null;
        ...
        private static bool TryRegisterIfKnownChest(Identity target)
        {
            ...
            if (!NascenceDungeon4RevealZones.IsKnownChestInstance(target.Instance))
            {
                return false;
            }
        ...
        private static List<TreasureLootSlot> BuildLoot(int containerInstance)
        {
            ...
            else
            {
                ...
                int poolIndex = rng.Next(FallbackLootPools.Length);
                defs = FallbackLootPools[poolIndex];
            }

            var items = new List<TreasureLootSlot>(defs.Count + 1);
            ...
            int crystalHigh;
            int crystalQl;
            if (NascenceTreasureShadowCrystalLoot.TryRollCrystal(
                containerInstance,
                generation,
                out crystalLow,
                out crystalHigh,
        ...
        // Capture 20260830-143801 HIGH: chest 0BAE7776 → Cracked Crystal (Shock Absorber) 220519/220519 QL70.
        private static readonly Dictionary<int, List<TreasureLootDefinition>> CapturedLootByContainer =
            ...
            {
                {
                    unchecked((int)0x0BAE7776),
                    new List<TreasureLootDefinition>
                    {
                        new TreasureLootDefinition { LowId = 220519, HighId = 220519, Quality = 70, Count = 1 }
                    }
                }
            };

        // Capture-family ACG treasure trash pools (D3-equivalent until per-chest D4 table is generated).
        ...
        private sealed class TreasureChestState
        {
            ...
            // Capture 20260830-143801: some chests spawn a trap mob on first open.
            internal bool TrapSpawned;
        }
