namespace AORebirth.Core.Playfields
{
    ...
    /// <summary>
    /// Capture 20260830-140240: Treasure (C749) Use opens container UI + loot — not credits+despawn.
    /// Loot item tables are provisional reuse of D2 until D3-specific drops are captured;
    /// chest identities/positions are from evidence=20260830-140240.
    /// Inventory observations: Container bag + handle 114+, flags 0x21, GenericCmd ACK Temp1=1.
    /// </summary>
    internal static class NascenceDungeon3TreasureLootService
    {
        ...
        private static readonly TimeSpan RespawnDelay = NascenceDungeon3Rules.TreasureRespawnDelay;
        ...
        internal static void ProcessDue(Playfield playfield, DateTime utcNow)
        {
            ...
            for (int i = 0; i < due.Count; i++)
            {
                ...
                NascenceDungeon3DoorReplay.RespawnTreasureChestInZone(
                    playfield,
                    pending.ZoneKey,
                ...
                LogUtil.Debug(
                    ...
                    string.Format(
                        ...
                        "NascenceDungeon3 treasure respawn chest={0:X8} zone={1:X16} pf={2}",
                        pending.ContainerIdentity.Instance,
                        pending.ZoneKey,
        ...
        internal static bool TryHandleUse(IZoneClient client, GenericCmdMessage message, Identity target)
        {
            ...
            if (!chest.TrapSpawned)
            {
                ...
                if (playfield != null)
                {
                    NascenceDungeon3Spawn.TrySpawnChestTrap(
                        playfield,
            ...
            LogUtil.Debug(
                ...
                string.Format(
                    System.Globalization.CultureInfo.InvariantCulture,
                    "NascenceDungeon3 treasure open chest={0:X8} handle={1} items={2} char={3}",
                    chest.ContainerIdentity.Instance,
            ...
            int targetPlacement)
        {
            ...
            if (!TryResolveLootRequest(sourceContainer, out chest, out lootSlot))
            {
                LogUtil.Debug(
                    ...
                    string.Format(
                        System.Globalization.CultureInfo.InvariantCulture,
                        "NascenceDungeon3 treasure loot unresolved source={0} type={1} instance={2:X8}",
                        sourceContainer,
            ...
            lock (Sync)
            {
                ...
                if (slot == null)
                {
                    LogUtil.Debug(
                        ...
                        string.Format(
                            System.Globalization.CultureInfo.InvariantCulture,
                            "NascenceDungeon3 treasure loot miss-slot chest={0:X8} handle={1} slot={2} source={3}",
                            chest.ContainerIdentity.Instance,
                            chest.InventoryHandle,
                            lootSlot,
                            sourceContainer));
            ...
            if (!TryCreateLootItem(slot, out item))
            {
                LogUtil.Debug(
                    ...
                    string.Format(
                        System.Globalization.CultureInfo.InvariantCulture,
                        "NascenceDungeon3 treasure loot bad-template chest={0:X8} low={1} high={2} ql={3}",
                        chest.ContainerIdentity.Instance,
                        slot.LowId,
                        slot.HighId,
                        slot.Quality));
            ...
            if (transfer.Status != CorpseLootInventoryTransferStatus.Success)
            {
                LogUtil.Debug(
                    ...
                    string.Format(
                        System.Globalization.CultureInfo.InvariantCulture,
                        "NascenceDungeon3 treasure loot transfer-fail chest={0:X8} status={1} invErr={2} ex={3}",
                        chest.ContainerIdentity.Instance,
                        transfer.Status,
            ...
            LogUtil.Debug(
                ...
                string.Format(
                    System.Globalization.CultureInfo.InvariantCulture,
                    "NascenceDungeon3 treasure loot ok chest={0:X8} low={1} high={2} invSlot={3}",
                    chest.ContainerIdentity.Instance,
                    item.LowID,
                    item.HighID,
                    transfer.TargetSlot));
            ...
            if (chest.Items.All(x => x.Looted))
            {
                ...
                long zoneKey = NascenceDungeon3RevealZones.ZoneKeyForContainer(chest.ContainerIdentity.Instance);
                lock (Sync)
                {
        ...
        private static bool TryCreateLootItem(TreasureLootSlot slot, out Item item)
        {
            ...
            catch (Exception ex)
            {
                LogUtil.Debug(
                    DebugInfoDetail.Error,
                    "NascenceDungeon3 treasure Item create failed id="
                    + templateId
                    + " err="
                    + ex.Message);
                item = null;
        ...
        private static bool TryRegisterIfKnownChest(Identity target)
        {
            ...
            if (!NascenceDungeon3RevealZones.IsKnownChestInstance(target.Instance))
            {
                return false;
            }
        ...
        private sealed class TreasureChestState
        {
            ...
            // Capture 20260830-140240: some chests spawn a trap mob on first open.
