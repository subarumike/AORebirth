namespace ZoneEngine_New.Core.Inventory
{
    ...
    public sealed partial class InventoryActionService
    {
        ...
        public bool TryUseVitalItem(Player player, Identity slot, Item item)
        {
            ...
            // its generic Sitting/InDuel / SelectedTargetType ToUse evaluator was not the accepted route.
                ...
                return TryCommit(player, changes,
                    ...
                    () =>
                    {
                        ...
                        player.Session?.Send(new TemplateActionMessage
                        {
                            ...
                            Quality = item.Quality, Placement = slot, Unknown1 = 1,
                            Action = TemplateActionType.Use, Unknown3 = 0, Unknown4 = 0
                        });
        ...
        /// <summary>
        /// OnUse LockSkill for non-vital items: same cooldown table as stims, so the client gets
        /// SpecialUnavailable / SpecialAvailable.
        /// </summary>
        internal bool TryApplyLockSkill(Player player, ItemSpell spell)
        {
            if (player.Session?.State != SessionState.InPlay || player.IsDead || player.IsPersistenceQuarantined)
                return false;
            if (!TryReadLockSkill(spell, out int skill, out int seconds))
                return false;
            return TryLockSkill(player, skill, seconds);
        }

        /// <summary>True when <paramref name="skill"/> is still locked; remaining seconds is at least 1.</summary>
        public bool TryGetSkillLockRemaining(Player player, int skill, out int remainingSeconds)
        {
            remainingSeconds = 0;
            if (player == null || skill <= 0)
                return false;

            DateTimeOffset now = Clock.GetUtcNow();
            lock (_skillLockGate)
            {
                if (!_skillLocks.TryGetValue((player, skill), out DateTimeOffset until) || until <= now)
                    return false;
                remainingSeconds = Math.Max(1, (int)Math.Ceiling((until - now).TotalSeconds));
                return true;
            }
        }

        /// <summary>Applies a skill lock and sends SpecialUnavailable (mail Computer Literacy postage lock).</summary>
        public bool TryLockSkill(Player player, int skill, int seconds)
        {
            if (player.Session?.State != SessionState.InPlay || player.IsDead || player.IsPersistenceQuarantined
                || skill <= 0 || seconds <= 0)
                return false;
            ...
            lock (_skillLockGate)
            {
                if (_skillLocks.TryGetValue((player, skill), out DateTimeOffset until) && until > now)
                    return false;
                _skillLocks[(player, skill)] = now.AddSeconds(seconds);
            }

            player.Session.Send(new CharacterActionMessage
            {
                Identity = player.Identity,
                Action = CharacterActionType.SpecialUnavailable,
        ...
        static bool TryResolveVitalEffects(Item item, VitalItemContent content, out int health, out int nano, out int skill, out int seconds)
        {
            ...
            if (item.SpellList.TryGetValue(EventType.OnUse, out var spells))
            {
                foreach (ItemSpell spell in spells)
                {
                    ...
                    {
                        if (!spell.TryReadInt(0, out int stat)
                    ...
                    {
                        if (!TryReadLockSkill(spell, out skill, out seconds)) return false;
                    }
                }
            }
        ...
        static bool TryReadLockSkill(ItemSpell spell, out int skill, out int seconds)
        {
            skill = 0;
            seconds = 0;
            if (!spell.TryReadInt(0, out int first) || !spell.TryReadInt(1, out int second))
                return false;
            if (Enum.IsDefined(typeof(CharacterStat), first))
            {
                skill = first;
                seconds = second;
            }
            ...
            {
                skill = second;
                if (!spell.TryReadInt(2, out seconds))
                    return false;
            }

            return seconds > 0 && Enum.IsDefined(typeof(CharacterStat), skill);
        }

        static bool TryResolveVitalHit(ItemSpell spell, out int amount)
        {
            amount = 0;
            if (!spell.TryReadInt(1, out int minimum) || minimum == int.MinValue) return false;
            int maximum = minimum;
            if (spell.ArgumentCount >= 3)
            {
                if (!spell.TryReadInt(2, out maximum) || maximum == int.MinValue) return false;
