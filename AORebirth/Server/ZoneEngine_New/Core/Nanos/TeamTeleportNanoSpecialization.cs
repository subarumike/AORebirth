...
/// <summary>Reusable selected-member and team teleport mechanics, configured by editable nano content.</summary>
public sealed class TeamTeleportNanoSpecialization(TeamService teams, Lazy<PlayfieldManager> playfields, NanoMechanicCatalog? mechanics = null)
    : INanoSpecialization, INanoCastContextSpecialization
{
    ...
    public bool Handles(int id) => _mechanics.TryGet(id, NanoMechanicKind.TeamTeleport, out _);
    public bool TryPrepare(Player caster, Player target, NanoDefinition nano, out NanoSpecializationPlan plan)
    {
        ...
        if (!ReferenceEquals(caster, target) || !Handles(nano.Id) || nano.DurationCentiseconds != 0
            || caster.Playfield == null || caster.Playfield.IsDisposed || !AllowedPlayfield(caster.Playfield.Identity.Instance, _mechanics.Get(nano.Id, NanoMechanicKind.TeamTeleport))
        ...
        return spell.FunctionType == (_mechanics.Get(nano.Id, NanoMechanicKind.TeamTeleport).RecipientMode == TeleportRecipientMode.Selected ? 53154 : 53155)
        ...
        Func<bool> casterStillCurrent, out Action afterCommit)
    {
        ...
        if (_mechanics.Get(nano.Id, NanoMechanicKind.TeamTeleport).RecipientMode == TeleportRecipientMode.Selected)
        {
        ...
        bool TryMember(int id, int slot, out Member? result)
        {
            ...
            if (!playfields.Value.FindPlayer(id, out Player player) || !Connected(player)
                ...
                || source.IsDisposed || !AllowedPlayfield(source.Identity.Instance, _mechanics.Get(nano.Id, NanoMechanicKind.TeamTeleport)) || !teams.AreTeammates(caster, player)) return false;
    ...
    internal static bool AllowedPlayfield(int id, NanoMechanicDefinition definition) => definition.AllowsPlayfield(id);
