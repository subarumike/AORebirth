namespace ZoneEngine_New.Tests
{
    ...
    public sealed class CellHeatSchedulerTests
    {
        ...
        [TestMethod]
        public void FightingTargetKeepsAcceptedCombatHotWithoutInventedAiState()
        {
            var scheduler = CreateOutdoorScheduler(out var grid);
            var npc = new CountingNpc(); npc.Stats.Set(CharacterStat.Health, 100);
            npc.SetFightingTarget(new Identity { Type = IdentityType.CanbeAffected, Instance = 42 });
            Place(grid, npc, new Vector3(20, 0, 20));
            scheduler.Tick([npc], 0.2); scheduler.Tick([npc], 0.2);
            Assert.AreEqual(2, npc.Ticks);
        }

        [TestMethod]
        public void StandaloneVendorKeepsColdTickWithoutAPlayer()
        {
            var scheduler = CreateOutdoorScheduler(out var grid); int ticks = 0;
            scheduler.ConfigureSpawnHooks([], _ => Assert.Fail("Vendor must not sleep"), _ => ticks++, () => { });
            var vendor = new VendingMachine(new Identity { Type = IdentityType.VendingMachine, Instance = 4 },
                new ItemTemplate { Id = 1, Quality = 1 });
            Place(grid, vendor, new Vector3(20, 0, 20)); scheduler.Tick([vendor], 0.2);
            Assert.AreEqual(1, ticks);
        }

        sealed class CountingNpc() : NpcCharacter(new Identity { Type = IdentityType.CanbeAffected, Instance = 9 }, new StubItemBuilder())
        {
            internal int Ticks;
            public override void Tick(double deltaTime) => Ticks++;
        }
