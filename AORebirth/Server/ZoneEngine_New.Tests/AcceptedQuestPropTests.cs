...
public sealed class AcceptedQuestPropTests
{
    ...
    static QuestPropService Create(AuthoredQuestTests.World w, bool complete = true)
    {
    ...
    public void BothAcceptedPropsMaterializeExactIdentityTemplatePositionAndEightCapturedStatsOnce()
    {
        ...
        foreach (var definition in w.Catalog.Content.Props)
        {
            ...
            Assert.AreEqual(Target(definition.Instance), wire.Identity);
            Assert.AreEqual(definition.Position[0], wire.Coordinate.X); Assert.AreEqual(definition.Position[1], wire.Coordinate.Y);
            Assert.AreEqual(definition.Position[2], wire.Coordinate.Z); Assert.AreEqual(6553, wire.Playfield);
    ...
    public void MissingCatalogOrConflictingExistingIdentityCannotMintAQuestCapability()
    {
        ...
        var impostor = new Dynel(Target(QuestPropFixture.RemainsInstance)) { Playfield = w.Player.Playfield };
        ...
        Assert.IsFalse(conflict.TryUseProp(w.Session, impostor.Identity, () => Assert.Fail("Unaccepted target emitted success.")));
    ...
    public void StrongboxRequiresExactLiveTargetAndDurableOwnedLockpickThenAcknowledgesAfterCommit()
    {
        ...
        var pick = w.Add(95577); w.Activate(AuthoredQuestFixture.Strongbox);
        var target = Target(QuestPropFixture.StrongboxInstance);
        w.Player.Position = new Vector3(3409.956f, 9.01f, 893.5452f);
        int acknowledgements = 0;
        w.Dao.BeforeCommit = _ => Assert.AreEqual(0, acknowledgements);
        Assert.IsTrue(service.TryUseItemOnProp(w.Session, Slot, target, () => acknowledgements++));
    ...
    {
        using var w = new AuthoredQuestTests.World(); var service = Create(w); service.Activate();
        w.Add(95577); w.Activate(AuthoredQuestFixture.Strongbox);
        var target = Target(QuestPropFixture.StrongboxInstance);
        Action noAck = () => Assert.Fail("Rejected target emitted success.");
        Assert.IsFalse(service.TryUseItemOnProp(w.Session, Slot, target, noAck));
        w.Player.Position = new Vector3(3409.956f, 9.01f, 893.5452f);
        Assert.IsFalse(service.TryUseItemOnProp(w.Session, new() { Type = IdentityType.Inventory, Instance = 65 }, target, noAck));
        w.Registry.Register(new Dynel(target) { Playfield = w.Player.Playfield, Position = w.Player.Position });
        Assert.IsFalse(service.TryUseItemOnProp(w.Session, Slot, target, noAck));
        service.Shutdown(); Assert.IsFalse(service.TryUseItemOnProp(w.Session, Slot, target, noAck));
        Assert.AreEqual(0, w.Dao.Calls);
    }
    ...
    {
        using var w = new AuthoredQuestTests.World(); var service = Create(w); service.Activate();
        w.Activate(AuthoredQuestFixture.FindThief);
        w.Player.Position = new Vector3(3424.016f, 0.01011355f, 887.8564f);
        var target = Target(QuestPropFixture.RemainsInstance); int acks = 0;
        using var foreign = new AuthoredQuestTests.World();
        Assert.IsFalse(service.TryUseProp(foreign.Session, target, () => acks++));
        w.Dao.BeforeCommit = _ => throw new InvalidOperationException("injected rollback after planning");
        Assert.IsFalse(service.TryUseProp(w.Session, target, () => acks++));
        ...
        Assert.IsTrue(service.TryUseProp(w.Session, target, () => acks++));
