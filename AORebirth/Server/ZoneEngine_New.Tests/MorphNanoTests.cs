namespace ZoneEngine_New.Tests
{
    ...
    public sealed class MorphNanoTests
    {
        ...
        public void Real_catalog_requirement_failure_and_unknown_head_branch_do_not_begin_cast()
        {
            ...
            Assert.IsFalse(f.Service.TryCast(f.Player, f.Nano.Id, f.Player.Identity));
            Assert.IsTrue(f.Session.Bodies.All(b => b is ChatTextMessage)); Assert.AreEqual(0, f.Store.Commits);
        }
        ...
        public void Remove_reverses_only_owned_overlay_and_preserves_original_base()
        {
            ...
            f.Player.Stats.Set(CharacterStat.DisplayCATMesh, 222); f.Restore(); f.Session.Bodies.Clear();
        ...
        {
            var f = new Fixture(281569); f.Restore(); f.Player.Motor.ApplyAction(MovementAction.SwitchToFly);
            ...
            Assert.AreEqual(0, f.Store.Rows.Count); Assert.AreEqual(1, f.Store.Commits);
            Assert.AreEqual(0, f.Player.Stats.GetOrZero(CharacterStat.MonsterData));
        ...
        {
            var f = new Fixture(270542); f.Restore(); f.Player.Stats.ClearBonuses();
        ...
        {
            var f = new Fixture(281569); f.Restore(); Assert.AreEqual(1, f.Player.Stats.GetOrZero(CharacterStat.IsVehicle));
            ...
            Assert.AreEqual(0, f.Session.Raw.Count); Assert.AreEqual(0, f.Store.Commits);
        }
        ...
        public void Fly_request_cannot_grant_authority_and_removal_demotes_to_previous_walk_mode()
        {
            ...
            f.Restore(); f.Player.Motor.ApplyAction(MovementAction.SwitchToFly); Assert.AreEqual(MovementState.Fly, f.Player.Motor.State);
        ...
        public void Existing_action_and_event_requirement_paths_remain_distinct_and_unknown_enforced_ops_fail()
        {
            ...
            Assert.IsFalse(NanoRequirements.Action(p, p, [falseOr])); // Cast admission is unavailable.
        ...
        public void Legacy_flying_allowed_operator_does_not_override_expansion_criterion_or_server_flight_authority()
        {
            ...
            var criterion = new ItemRequirement { ChildOperator = (int)Operator.And, Operator = (int)Operator.FlyingAllowed,
                Target = (int)ItemTarget.Self };
            Assert.IsFalse(NanoRequirements.Action(f.Player, f.Player, [criterion]));
            ...
            Assert.AreEqual(MovementState.Run, f.Player.Motor.State);
            Assert.AreEqual(0, f.Store.Commits); Assert.IsTrue(f.Session.Bodies.All(b => b is ChatTextMessage));
        }
        ...
        private sealed class Fixture
        {
            ...
            public void Restore()
            {
                ...
                Store.Rows = [new ActiveNanoRecord(Nano.Id, Nano.Strain, 42, Nano.DurationCentiseconds,
                    Now.AddMilliseconds(Nano.DurationCentiseconds * 10L).Ticks)];
